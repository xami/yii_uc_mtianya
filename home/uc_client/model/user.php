...
class usermodel {
	...
	function check_username($username) {
		...
		$len = strlen($username);
		if($len > 15 || $len < 3 || preg_match("/\s+|^channel:\\con\\con|[%,\*\"\s\<\>\&]|$guestexp/is", $username)) {
			return FALSE;
		} else {
			return TRUE;
		}
	...
	function check_usernamecensor($username) {
		...
		$censorexp = '/^('.str_replace(array('\\*', "\r\n", ' '), array('.*', '|', ''), preg_quote(($censorusername = trim($censorusername)), '/')).')$/item';
	...
	function check_emailaccess($email) {
		...
		$accessexp = '/('.str_replace("\r\n", '|', preg_quote(trim($accessemail), '/')).')$/item';
		$censorexp = '/('.str_replace("\r\n", '|', preg_quote(trim($censoremail), '/')).')$/item';
