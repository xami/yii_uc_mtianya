...
function feed_publish($id, $idtype, $add=0) {
		...
		case 'eventid':
			...
			if($value = $_SGLOBAL['db']->fetch_array($query)) {
				...
				$setarr['body_data'] = array(
					...
					'starttime' => sgmdate('m-d H:item', $value['starttime']),
					'endtime' => sgmdate('m-d H:item', $value['endtime'])
				);
				//����
				if($value['poster']) {
	...
	if($setarr['icon']) {
		...
		$setarr['hash_data'] = md5($setarr['title_template']."\t".$setarr['title_data']."\t".$setarr['body_template']."\t".$setarr['body_data']);//�ϲ�hash
