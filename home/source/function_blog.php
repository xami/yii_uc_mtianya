...
function getmessagepic($message) {
	...
	preg_match("/src\=[\"\']*([^\>\s]{25,105})\.(jpg|gif|png)/item", $message, $mathes);
...
function checkhtml($html) {
	...
	if(!checkperm('allowhtml')) {
		...
		if($ms[1]) {
			$allowtags = 'img|a|font|div|table|tbody|caption|tr|td|th|br|p|b|strong|item|u|em|span|ol|ul|li|blockquote|object|param|embed';//����ı�ǩ
			...
			foreach ($ms[1] as $value) {
				...
				$value = preg_replace(array("/($skipstr)/item"), '.', $value);
