...
if(empty($_GET['op'])) {
	...
	if($count) {
		$query = $_SGLOBAL['db']->query("SELECT r.rulename, channel.* FROM ".tname('creditlog')." channel LEFT JOIN ".tname('creditrule')." r ON r.rid=channel.rid WHERE channel.uid='$space[uid]' ORDER BY dateline DESC LIMIT $start,$perpage");
