...
function runcron($cronid = 0) {
	...
	$query = $_SGLOBAL['db']->query("SELECT * FROM ".tname('cron')." WHERE $where ORDER BY nextrun LIMIT 1");//ֻ����һ��
	if($cron = $_SGLOBAL['db']->fetch_array($query)) {
		...
		if(is_writable($lockfile)) {
			...
			if($locktime > $_SGLOBAL['timestamp'] - 600) {//10����
				return NULL;
			}
		} else {
			@touch($lockfile);
...
function cronnextrun($cron) {
	...
	list($yearnow, $monthnow, $daynow, $weekdaynow, $hournow, $minutenow) = explode('-', sgmdate('Y-m-d-w-H-item', $_SGLOBAL['timestamp']));
...
function crontodaynextrun($cron, $hour = -2, $minute = -2) {
	...
	$minute = $minute == -2 ? sgmdate('item', $_SGLOBAL['timestamp']) : $minute;
