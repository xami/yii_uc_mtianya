...
function submitcheck($var) {
	if(!empty($_POST[$var]) && $_SERVER['REQUEST_METHOD'] == 'POST') {
		if((empty($_SERVER['HTTP_REFERER']) || preg_replace("/https?:\/\/([^\:\/]+).*/item", "\\1", $_SERVER['HTTP_REFERER']) == preg_replace("/([^\:]+).*/", "\\1", $_SERVER['HTTP_HOST'])) && $_POST['formhash'] == formhash()) {
			return true;
		} else {
...
function runlog($file, $log, $halt=0) {
	...
	$log = sgmdate('Y-m-d H:item:s', $_SGLOBAL['timestamp'])."\t$type\t".getonlineip()."\t$_SGLOBAL[supe_uid]\t{$nowurl}\t".str_replace(array("\r", "\n"), array(' ', ' '), trim($log))."\n";
...
function sstrtotime($string) {
	...
	if($string) {
		...
		if(gmdate('H:item', $_SGLOBAL['timestamp'] + $_SCONFIG['timeoffset'] * 3600) != date('H:item', $_SGLOBAL['timestamp'])) {
...
function ob_out() {
	...
	if($_SCONFIG['allowrewrite']) {
		...
		$preg_searchs[] = "/\<a href\=\"space.php\"/item";
		$preg_searchs[] = "/\<a href\=\"network\.php\?ac\=([a-z0-9\=\&]+?)\"/ie";
		$preg_searchs[] = "/\<a href\=\"network.php\"/item";
...
function isholddomain($domain) {
	...
	if(preg_match("/^[^a-z]/item", $domain)) return true;
	...
	foreach ($holdmainarr as $value) {
		if(strpos($value, '*') === false) {
			...
			if(@preg_match("/$value/item", $domain)) {
				$ishold = true;
				break;
			}
		}
...
function mktarget($html) {
	...
	if($html && $_SCONFIG['feedtargetblank']) {
		$html = preg_replace("/<a(.+?)href=([\'\"]?)([^>\s]+)\\2([^>]*)>/item", '<a target="_blank" \\1 href="\\3" \\4>', $html);
	}
	return $html;
}
...
function topic_get($topicid) {
	...
	if($topicid) {
		...
		if($topic = $_SGLOBAL['db']->fetch_array($query)) {
			...
			$topic['lastpost'] = sgmdate('Y-m-d H:item', $topic['lastpost']);
			$topic['dateline'] = sgmdate('Y-m-d H:item', $topic['dateline']);
			$topic['allowjoin'] = $topic['endtime'] && $_SGLOBAL['timestamp']>$topic['endtime']?0:1;
			$topic['endtime'] = $topic['endtime']?sgmdate('Y-m-d H:item', $topic['endtime']):'';
...
function ckspacelog() {
	...
	if($value = $_SGLOBAL['db']->fetch_array($query)) {
		...
		$expiration = sgmdate('Y-m-d H:item', $value['expiration']);
