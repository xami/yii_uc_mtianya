...
if($_GET['op'] == 'delete') {
	...
	switch ($type) {
		...
		case 'event':
			...
			$arr['body_data'] = array(
				...
				'eventtime' => sgmdate('m-d H:item', $event['starttime']) . " - " . sgmdate("m-d H:item", $event['endtime']),
	...
	if(submitcheck('sharesubmit')) {
		...
		if($type == 'link') {
			...
			if($link) {
				if(!preg_match("/^(http|ftp|https|mms)\:\/\/.{4,300}$/item", $link)) $link = '';
			}
			if(empty($link)) {
			...
			if(preg_match("/(youku.com|youtube.com|5show.com|ku6.com|sohu.com|mofile.com|sina.com.cn)$/item", $parseLink['host'], $hosts)) {
