...
if(submitcheck('threadsubmit')) {
	...
	$_POST['message'] = preg_replace("/\<div\>\<\/div\>/item", '', $_POST['message']);
	...
	//��������
	if($uploads) {
		preg_match_all("/\<img\s.*?\_uchome\_localimg\_([0-9]+).+?src\=\"(.+?)\"/item", $message, $mathes);
		if(!empty($mathes[1])) {
	...
	if(empty($_POST['tid'])) {
		...
		if(checkperm('edittrail')) {
			$message = $message.saddslashes(cplang('thread_edit_trail', array($_SGLOBAL['supe_username'], sgmdate('Y-m-d H:item:s'))));
	...
	if(checkperm('edittrail') || ($post['uid'] && $post['uid'] != $space['uid'])) {
		$message = $message.saddslashes(cplang('thread_edit_trail', array($_SN[$_SGLOBAL['supe_uid']], sgmdate('Y-m-d H:item:s'))));
	}
