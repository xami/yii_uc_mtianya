...
class xmlrpc {
	...
	function newMediaObject($uid, $username, $password, $mediaobject = array()) {
		...
		if(!preg_match("/^(http\:\/\/|\/)/item", $struct['url'])) $struct['url'] = $this->siteUrl.$struct['url'];
		return $struct;
	...
	function getPost($postid, $username, $password) {
		...
		$postid = intval($postid);
		$uid = $this->member['uid'];
		$query = $this->db->query("SELECT bf.message, b.blogid, b.subject, b.dateline, channel.classid, channel.classname FROM ".tname('blog')." b LEFT JOIN ".tname('blogfield')." bf ON bf.blogid=b.blogid LEFT JOIN ".tname('class')." channel ON channel.classid=b.classid WHERE b.blogid='$postid' AND b.uid='$uid'");
		if($item = $this->db->fetch_array($query)) {
			$item['dateline'] = sgmdate('Ymd\TH:item:s', $item['dateline']);
			$struct = array(
				'userid' => $uid,
