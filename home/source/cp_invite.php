...
if(submitcheck('emailinvite')) {
	...
	foreach($mails as $key => $value) {
		...
		if($reward['credit']) {
			...
			$mailvar[4] = "{$siteurl}invite.php?u=$space[uid]&amp;channel=$invite_code{$inviteapp}";
			if($appid) {
				$mailvar[6] = $appinfo['appname'];
			}
			createmail($value, $mailvar);
	...
	if(submitcheck('resendsubmit')) {
		...
		if($value = $_SGLOBAL['db']->fetch_array($query)) {
			if($reward['credit']) {
				...
				$inviteurl = "{$siteurl}invite.php?u=$space[uid]&amp;channel=$invite_code";
			}
			$mailvar[4] = $inviteurl;
	...
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
		...
		if($value['fuid']) {
			...
			if($reward['credit']) {
				...
				$inviteurl = "{$siteurl}invite.php?u=$space[uid]&amp;channel=$invite_code{$inviteapp}";
			}
			if($value['type']) {
				$maillist[] = array(
					'email' => $value['email'],
