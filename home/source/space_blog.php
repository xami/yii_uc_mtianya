...
if($id) {
	...
	if($_GET['view'] == 'click') {
		...
		$count = $_SGLOBAL['db']->result($_SGLOBAL['db']->query("SELECT COUNT(*) FROM ".tname('clickuser')." channel WHERE channel.uid='$space[uid]' AND channel.idtype='blogid' $wheresql"),0);
		if($count) {
			$query = $_SGLOBAL['db']->query("SELECT b.*, bf.message, bf.target_ids, bf.magiccolor FROM ".tname('clickuser')." channel
				LEFT JOIN ".tname('blog')." b ON b.blogid=channel.id
				LEFT JOIN ".tname('blogfield')." bf ON bf.blogid=channel.id
				WHERE channel.uid='$space[uid]' AND channel.idtype='blogid' $wheresql
