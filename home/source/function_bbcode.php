...
function bbcode($message, $parseurl=0) {
	...
	if(empty($_SGLOBAL['search_exp'])) {
		$_SGLOBAL['search_exp'] = array(
			"/\s*\[quote\][\n\r]*(.+?)[\n\r]*\[\/quote\]\s*/is",
			"/\[url\]\s*(https?:\/\/|ftp:\/\/|gopher:\/\/|news:\/\/|telnet:\/\/|rtsp:\/\/|mms:\/\/|callto:\/\/|ed2k:\/\/){1}([^\[\"']+?)\s*\[\/url\]/item",
			"/\[em:([0-9]+):\]/item",
		);
		...
		$_SGLOBAL['search_str'] = array('[b]', '[/b]','[item]', '[/item]', '[u]', '[/u]');
		$_SGLOBAL['replace_str'] = array('<b>', '</b>', '<item>','</item>', '<u>', '</u>');
	}
...
function parseurl($message) {
	return preg_replace("/(?<=[^\]a-z0-9-=\"'\\/])((https?|ftp|gopher|news|telnet|mms|rtsp):\/\/)([a-z0-9\/\-_+=.~!%@?#%&;:$\\()|]+)/item", "[url]\\1\\3[/url]", ' '.$message);
}
...
function html2bbcode($message) {
	...
	if(empty($_SGLOBAL['html_s_exp'])) {
		...
		$_SGLOBAL['html_s_str'] = array('<b>', '</b>', '<item>','</item>', '<u>', '</u>', '&nbsp; &nbsp; &nbsp; &nbsp; ', '&nbsp; &nbsp;', '&nbsp;&nbsp;', '&lt;', '&gt;', '&amp;');
		$_SGLOBAL['html_r_str'] = array('[b]', '[/b]','[item]', '[/item]', '[u]', '[/u]', "\t", '   ', '  ', '<', '>', '&');
	}	
