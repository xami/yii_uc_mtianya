...
function pic_save($FILE, $albumid, $title, $topicid=0) {
	...
	$albumfriend = 0;
	if($albumid) {
		preg_match("/^new\:(.+)$/item", $albumid, $matchs);
		if(!empty($matchs[1])) {
...
function invite_get($uid, $code) {
	...
	$invitearr = array();
	if($uid && $code) {
		$query = $_SGLOBAL['db']->query("SELECT item.*, s.username, s.name, s.namestatus
			FROM ".tname('invite')." item
			LEFT JOIN ".tname('space')." s ON s.uid=item.uid
			WHERE item.uid='$uid' AND item.code='$code' AND item.fuid='0'");
...
function picurl_get($picurl, $maxlenth='200') {
	...
	if($picurl) {
		if(preg_match("/^http\:\/\/.{5,$maxlenth}\.(jpg|gif|png)$/item", $picurl)) return $picurl;
	}
	return '';
}
