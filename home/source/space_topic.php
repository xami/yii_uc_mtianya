...
if(empty($topicid)) {
	...
	if($count) {
		while ($value = $_SGLOBAL['db']->fetch_array($query)) {
			...
			$value['lastpost'] = sgmdate('m-d H:item', $value['lastpost']);
			$value['dateline'] = sgmdate('m-d H:item', $value['dateline']);
			$value['endtime'] = $value['endtime']?sgmdate('m-d H:item', $value['endtime']):'';
