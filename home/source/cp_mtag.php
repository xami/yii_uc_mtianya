...
if($_GET['op'] == 'manage') {
	...
	if(submitcheck('outsubmit')) {
		...
		if($mtag['status'] != -9) {
			mtag_out($mtag, array($_SGLOBAL['supe_uid']));//�˳�
		}
	...
	$count = 0;
	$invites = array();
	$query = $_SGLOBAL['db']->query("SELECT mtag.*, item.* FROM ".tname('mtaginvite')." item
		LEFT JOIN ".tname('mtag')." mtag ON mtag.tagid=item.tagid
		WHERE item.uid='$_SGLOBAL[supe_uid]' ORDER BY item.dateline DESC");
	while ($value = $_SGLOBAL['db']->fetch_array($query)) {
...
function mtag_join($type, $key, $fieldid=0) {
	...
	if($mtag['joinperm'] == 2) {
		...
		if($modcount) {
			$mtag['grade'] = $setarr['grade'] = 0;//�Ѿ���Ⱥ��
		} else {
			$mtag['grade'] = $setarr['grade'] = $field['manualmoderator']?0:9;//�Զ�ΪȺ��
		}
		//�¼�֪ͨ
		if(ckprivacy('mtag', 1)) {
			$fs = array();
