...
//note ������붨��
define('EXPLOR_SUCCESS', 0);//note �����ɹ�
define('IMPORT_SUCCESS', 0);//note ����ɹ�
define('DELETE_SQLPATH_SUCCESS', 4);//note ɾ����ļ��ɹ�
define('MKDIR_ERROR', 1);//note ����Ŀ¼ʧ��
define('DATABASE_EXPORT_FILE_INVALID', 2);//note �����ļ�д��ʧ��
define('RUN_SQL_ERROR', 3);//note sqlִ�д���
define('SQLPATH_NULL_NOEXISTS', 4);//note sqlpathΪ�ջ���Ŀ¼������
define('SQLPATH_NOMATCH_BAKFILE', 5);//note ָ��sqlpahtû���ҵ��Ϸ��ı����ļ�
define('BAK_FILE_LOSE', 6);//note �����ļ�ȱʧ
define('DIR_NO_EXISTS', 7);//note ָ������Ŀ¼������
define('DELETE_DUMPFILE_ERROR', 8);//note ɾ��ָ������ݿⱸ���ļ�����
define('DB_API_NO_MATCH', 9);//note ���ݽӿڳ����ݲ�֧�ִ���Ӧ�õı���

$sizelimit = 2000;//note �־�Ĵ�С����λK��Ĭ��Ϊ2000K���벻Ҫ���õ�̫��һ�泬��php�ɲ������ڴ�
$usehex = true;//note ʹ��ʮ����ƣ��������̶ȱ����ַ����⡣
...
if($apptype == 'discuz') {//note discuz
	...
	define('BACKUP_DIR', ROOT_PATH.'forumdata/');//note ��ݿⱸ���ļ�����·��
	$tablepre = $tablepre;//note ��ǰ׺��ֵ����ͬ�Ĳ�Ʒ���޸Ĵ˱��ʽ
	if(empty($dbcharset)) {
	...
	define('BACKUP_DIR', ROOT_PATH.'./data/');//note ��ݿⱸ���ļ�����·��
	$tablepre = $_SC['tablepre'];//note ��ǰ׺��ֵ����ͬ�Ĳ�Ʒ���޸Ĵ˱��ʽ
	$dbcharset = $_SC['dbcharset'];
	$db->connect($_SC['dbhost'], $_SC['dbuser'], $_SC['dbpw'], $_SC['dbname'], $dbcharset, $_SC['pconnect'], $tablepre);

} elseif($apptype == 'ucenter') {//note ucenter

	define('BACKUP_DIR', ROOT_PATH.'./data/backup/');//note ��ݿⱸ���ļ�����·��
	$tablepre = UC_DBTABLEPRE;//note ��ǰ׺��ֵ����ͬ�Ĳ�Ʒ���޸Ĵ˱��ʽ
	...
	define('BACKUP_DIR', ROOT_PATH.'./data/backup/');//note ��ݿⱸ���ļ�����·��
	$tablepre = DB_PREFIX;//note ��ǰ׺��ֵ����ͬ�Ĳ�Ʒ���޸Ĵ˱��ʽ
	...
	define('BACKUP_DIR', ROOT_PATH.'data/backup/');//note ��ݿⱸ���ļ�����·��
	$tablepre = $prefix;//note ��ǰ׺��ֵ����ͬ�Ĳ�Ʒ���޸Ĵ˱��ʽ
	$dbcharset = 'utf8';
	$db->connect($db_host, $db_user, $db_pass, $db_name, $dbcharset, 0, $tablepre);

}

if($get['method'] == 'export') {//note ��������

	$db->query('SET SQL_QUOTE_SHOW_CREATE=0', 'SILENT');

	$time = date("Y-m-d H:item:s", $timestamp);

	$tables = array();
	$tables = arraykeys2(fetchtablelist($tablepre), 'Name');

	if($apptype == 'discuz') {//note discuz�ı�����Ҫ���ݲ������������
	...
	if(!isset($get['sqlpath']) || empty($get['sqlpath'])) {//note ��û��ָ�����Ŀ¼�����趨��ű�����ݵ�Ŀ¼
	...
	if(!isset($get['backupfilename']) || empty($get['backupfilename'])) {//note ��û��ָ�������ļ������趨������ݵ��ļ���
...
} elseif($get['method'] == 'import') {//note ���뱸�ݣ����ָ����
...
} elseif($get['method'] == 'ping') {//note ̽��ĳ��ָ��Ŀ¼���Ƿ��б������
...
} elseif($get['method'] == 'list') {//note �г����õı���Ŀ¼����xml��ʽ����
...
} elseif($get['method'] == 'view') {//note ��ʾ�����ĳһ������Ŀ¼�µ�����
...
} elseif($get['method'] == 'delete') {//note ɾ������ĳһ������Ŀ¼�µı����ļ���Ŀ¼ͬʱҲɾ��
...
function syntablestruct($sql, $version, $dbcharset) {
	...
	if($sqlversion === $version) {
		...
		//note ���Ϊ�߰汾������������ת�����ַ�������滻��
		return $sqlversion && $dbcharset ? preg_replace(array('/ character set \w+/item', '/ collate \w+/item', "/DEFAULT CHARSET=\w+/is"), array('', '', "DEFAULT CHARSET=$dbcharset"), $sql) : $sql;
	}

	//note ����ת��
	if($version) {
		return preg_replace(array('/TYPE=HEAP/item', '/TYPE=(\w+)/is'), array("ENGINE=MEMORY DEFAULT CHARSET=$dbcharset", "ENGINE=\\1 DEFAULT CHARSET=$dbcharset"), $sql);

	//note ����ת��
	} else {
		return preg_replace(array('/character set \w+/item', '/collate \w+/item', '/ENGINE=MEMORY/item', '/\s*DEFAULT CHARSET=\w+/is', '/\s*COLLATE=\w+/is', '/ENGINE=(\w+)(.*)/is'), array('', '', 'ENGINE=HEAP', '', '', 'TYPE=\\1\\2'), $sql);
	}
}
