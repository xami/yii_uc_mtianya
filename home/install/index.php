...
if (submitcheck('ucsubmit')) {
	...
	if(empty($ucapi) || !preg_match("/^(http:\/\/)/item", $ucapi)) {
...
END;
	...
	$app_url = strtolower(substr($_SERVER['SERVER_PROTOCOL'], 0, strpos($_SERVER['SERVER_PROTOCOL'], '/'))).'://'.$_SERVER['HTTP_HOST'].preg_replace("/\/*install$/item", '', substr($uri, 0, strrpos($uri, '/install')));
...
END;
	...
	foreach ($keys as $value) {
		...
		$configcontent = preg_replace("/define\('UC_".$upkey."'\s*,\s*'.*?'\)/item", "define('UC_".$upkey."', '".$_POST['uc'][$value]."')", $configcontent);
	}
