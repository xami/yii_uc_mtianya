...
if(submitcheck('valuesubmit')) {
	...
	$setarr['htmlcode'] = addslashes(preg_replace("/href\=\"(?!http\:\/\/)(.+?)\"/item", 'href="'.getsiteurl().'\\1"', stripslashes($setarr['htmlcode'])));
	updatetable('block', $setarr, array('bid'=>$bid));
...
function sub_getblocksql($sql) {
	if(strlen($sql)> 15) {
		$searchs = array("/^(select)/item", "/(\s+limit.+)/item");
		$replaces = array('', '');
...
function sub_getcolumn($creatsql) {
	...
	foreach ($arr as $value) {
		...
		if(!preg_match("/(CREATE|PRIMARY|KEY|\))/item", $value)) {
			$cols[] = $value;
		}
	}
	return $cols;
