...
if(submitcheck('cronsubmit')) {
	...
	if(empty($cronid)) {
		...
		$setarr['cronid'] = inserttable('cron', $setarr, 1);//����cronid
	} else {
		//UPDATE
...
if($_GET['op'] == 'edit') {
	...
	while($cron = $_SGLOBAL['db']->fetch_array($query)) {
		...
		$cron['lastrun'] = $cron['lastrun'] ? sgmdate("Y-m-d H:item:s", $cron['lastrun']) : 'N/A';
		$cron['nextrun'] = $cron['available'] && $cron['nextrun'] ? sgmdate("Y-m-d H:item:s", $cron['nextrun']) : 'N/A';
		$list[] = $cron;
	}
