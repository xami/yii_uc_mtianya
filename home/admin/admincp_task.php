...
if(submitcheck('tasksubmit')) {
	...
	if(empty($taskid)) {
		//ADD
		inserttable('task', $setarr);//����taskid
	} else {
		//UPDATE
...
if($_GET['op'] == 'edit') {
	...
	if($thevalue = $_SGLOBAL['db']->fetch_array($query)) {
		$thevalue['starttime'] = $thevalue['starttime']?sgmdate('Y-m-d H:item:s', $thevalue['starttime']):'';
		$thevalue['endtime'] = $thevalue['endtime']?sgmdate('Y-m-d H:item:s', $thevalue['endtime']):'';
	}
	...
	while($task = $_SGLOBAL['db']->fetch_array($query)) {
		$task['starttime'] = $task['starttime']?sgmdate("Y-m-d H:item:s", $task['starttime']) : 'N/A';
		$task['endtime'] = $task['endtime']?sgmdate("Y-m-d H:item:s", $task['endtime']) : 'N/A';
