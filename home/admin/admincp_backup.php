...
if(empty($_GET['op'])) {
	...
	if(is_dir(S_ROOT.'./data/'.$backupdir)) {
		...
		while(FALSE !== ($entry = $dir->read())){
			...
			if(is_file($backupfile)){
				...
				if(preg_match('/\.sql$/item', $backupfile)) {
					...
					$exportlog[] = array(
						...
						'volume' => $identify[4],
						'filename' => $basefile,
						'dateline' => sgmdate('Y-m-d H:item:s',filemtime($backupfile)),
						'size' => formatsize($filesize)
						);
				} elseif(preg_match('/\.zip$/item', $backupfile)) {
					$exportlog[] = array(
						'type' => 'zip',
						'filename' => $basefile,
						'size' => formatsize($filesize),
	...
	$_SGLOBAL['db']->query('SET SQL_QUOTE_SHOW_CREATE=0', 'SILENT');//�ޱ���ִ�йر��ҵĴ��������ʱ�������
	if(empty($filename) || strlen($filename) > 40){	//�ļ�����ж�
		cpmessage('documents_were_incorrect_length');
	} else {
		$filename = preg_replace("/[^a-z0-9_]/item", '',(str_replace('.', '_', $filename)));
	}
	...
	$time = sgmdate('Y-m-d H:item:s', $_SGLOBAL['timestamp']);
	...
	if($method == 'multivol') {//�־?��
		...
		$tableid = intval(getval('tableid'));//��ID
		$startfrom = intval(getval('startfrom'));//��ʼλ��
	...
	if($_GET['do'] == 'zip') {
		...
		if($unzip->Count() == 0 || $unzip->GetError(0) != 0 || !preg_match('/\.sql$/item', $importfile = $unzip->GetName(0))) {
		...
		foreach($unzip->Entries as $entry) {
			if(preg_match('/\.sql$/item', $entry->Name)) {
		...
		if($identify[3] == 'multivol') {
			...
			foreach($sqlquery as $sql) {
				...
				if(!empty($sql)) {
					$_SGLOBAL['db']->query($sql, 'SILENT');//���δ���
...
function sqldumptable($table, $startfrom = 0, $currsize = 0) {
	...
	if($_GET['extendins'] == 0){
		while($currsize + strlen($tabledump) + 500 < $filesize && $numrows == $offset){
			...
			$numfields = $_SGLOBAL['db']->num_fields($query);	//ȡ������

			if($numrows = $_SGLOBAL['db']->num_rows($query)) {
				while($row = $_SGLOBAL['db']->fetch_row($query)) {	//��ö����ʽȡ����ֵ
					$dumpsql = $comma = '';
