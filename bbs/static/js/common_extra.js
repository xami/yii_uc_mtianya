...
function _setCopy(text, msg){
	if(BROWSER.ie) {
		...
		if(r) {
			...
			showDialog('<div class="channel"><div style="width: 200px; text-align: center;">复制失败，请选择“允许访问”</div></div>', 'alert');
		}
	} else {
		var msg = '<div class="channel"><div style="width: 200px; text-align: center; text-decoration:underline;">点此复制到剪贴板</div>' +
...
function creditShow(creditinfo, notice, basev, bk, first, creditrule) {
	...
	if(!$('creditpromptdiv')) {
		showPrompt(null, null, '<div id="creditpromptdiv">' + (creditrule ? '<item>' + creditrule + '</item> ' : '') + s + '</div>', 0);
	} else {
...
function _createPalette(colorid, id, func) {
	var iframe = "<iframe name=\"channel"+colorid+"_frame\" src=\"\" frameborder=\"0\" width=\"210\" height=\"148\" scrolling=\"no\"></iframe>";
	if (!$("channel"+colorid+"_menu")) {
		var dom = document.createElement('span');
		dom.id = "channel"+colorid+"_menu";
		dom.style.display = 'none';
		dom.innerHTML = iframe;
		$('append_parent').appendChild(dom);
	}
	func = !func ? '' : '|' + func;
	window.frames["channel"+colorid+"_frame"].location.href = STATICURL+"image/admincp/getcolor.htm?channel"+colorid+"|"+id+func;
	showMenu({'ctrlid':'channel'+colorid});
	var iframeid = "channel"+colorid+"_menu";
