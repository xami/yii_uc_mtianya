...
function sethtml(id) {
	...
	btn.id = id + '_c';
	btn.type = 'button';
	btn.className = 'htmlbtn channel';
	if(curvalue.search(/<font/ig) !== -1) {
		btn.className = 'htmlbtn channel current';
	}
	...
	btn.id = id + '_i';
	btn.type = 'button';
	btn.className = 'htmlbtn item';
	if(curvalue.search(/<i>/ig) !== -1) {
		btn.className = 'htmlbtn item current';
	}
	btn.onclick = function() {
		var oldvalue = $(id).innerHTML;
		$(id).innerHTML = preg_replace(['<item>', '</item>'], '', $(id).innerHTML);
		if(oldvalue == $(id).innerHTML) {
			$(id + '_i').className = 'htmlbtn item current';
			$(id).innerHTML = '<item>' + $(id).innerHTML + '</item>';
		} else {
			$(id + '_i').className = 'htmlbtn item';
		}
		$(id + '_v').value = $(id).innerHTML;
	};
	$(id).parentNode.appendChild(btn);
...
function sethtml_color(color) {
	...
	if(color != 'transparent') {
		$(sethtml_id + '_c').className = 'htmlbtn channel current';
		$(sethtml_id).innerHTML = '<font color=' + color + '>' + $(sethtml_id).innerHTML + '</font>';
	} else {
		$(sethtml_id + '_c').className = 'htmlbtn channel';
	}
