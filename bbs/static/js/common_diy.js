...
(function(){
	...
	Drag = function () {
		...
		this.tabContentClass = 'tb-channel';
		this.moving = 'moving';
	...
	Drag.prototype = {
		...
		_replaceFrameColumn : function (ele,flag) {
			...
			for (var i in children) {
				if (Util.hasClass(children[i], this.moveableColumn)) {
					...
					className = className.replace(' col-channel', ' '+frameClassName+'-channel');
		...
		deleteFrame : function (name) {
			if (typeof name == 'string') {
				if (typeof window['channel'+name+'_frame'] == 'object' && !BROWSER.ie) delete window['channel'+name+'_frame'];
			} else {
				for(var i = 0,L = name.length;i < L;i++) {
					if (typeof window['channel'+name[i]+'_frame'] == 'object' && !BROWSER.ie) delete window['channel'+name[i]+'_frame'];
				}
 			}
		},
