...
class pmmodel {
	...
	function getpmbypmid($uid, $pmid) {
		...
		$arr = array();
		$pm = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."pm_indexes item LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON t.plid=item.plid WHERE item.pmid='$pmid'");
	...
	function deletepmbypmid($uid, $pmid) {
		...
		$index = $this->db->fetch_first("SELECT * FROM ".UC_DBTABLEPRE."pm_indexes item LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON item.plid=t.plid WHERE item.pmid='$pmid'");
		if($index['pmtype'] != 1) {
...
/*
	function deletepmbypmids($uid, $pmids) {
		...
		$existplid = array();
		$query = $this->db->query("SELECT * FROM ".UC_DBTABLEPRE."pm_indexes item LEFT JOIN ".UC_DBTABLEPRE."pm_lists t ON item.plid=t.plid WHERE item.pmid IN (".$this->base->implode($pmids).") AND t.pmtype=1");
