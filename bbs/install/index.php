...
if($method == 'show_license') {
	...
	if($submit) {
		...
		if($username && $email && $password) {
			if(strlen($username) > 15 || preg_match("/^$|^channel:\\con\\con$|　|[,\"\s\t\<\>&]|^Guest/is", $username)) {
		...
		$siteuniqueid = 'DX'.$chars[date('y')%60].$chars[date('n')].$chars[date('j')].$chars[date('G')].$chars[date('item')].$chars[date('s')].substr(md5($onlineip.$timestamp), 0, 4).random(4);
