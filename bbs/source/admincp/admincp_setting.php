...
EOT;
		...
		$checktimeformat = array($setting['timeformat'] == 'H:item' ? 24 : 12 => 'checked');
		...
		$setting['watermarktext']['fontpath'] = str_replace(array('channel/', 'en/'), '', $setting['watermarktext']['fontpath']);
		...
		$dir = opendir(DISCUZ_ROOT.'./static/image/seccode/font/channel');
		while($entry = readdir($dir)) {
			if(in_array(strtolower(fileext($entry)), array('ttf', 'ttc'))) {
				$fontlist['portal'] .= '<option value="'.$entry.'"'.($entry == $setting['watermarktext']['fontpath']['portal'] ? ' selected>' : '>').$entry.'</option>';
				$fontlist['forum'] .= '<option value="'.$entry.'"'.($entry == $setting['watermarktext']['fontpath']['forum'] ? ' selected>' : '>').$entry.'</option>';
	...
	if(isset($settingnew['seccodedata'])) {
		...
		if($settingnew['seccodedata']['type'] == 0 || $settingnew['seccodedata']['type'] == 2) {
			...
			$seccoderoot = 'static/image/seccode/font/channel/';
		}
		if($seccoderoot) {
	...
	if(isset($settingnew['timeformat'])) {
		$settingnew['timeformat'] = $settingnew['timeformat'] == '24' ? 'H:item' : 'h:item A';
	}
	...
	if($operation == 'mobile') {
		...
		$settingnew['mobile_arr']['mobilecomefrom'] = preg_replace(array("/\son(.*)=[\'\"](.*?)[\'\"]/item"), '', strip_tags($settingnew['mobile']['mobilecomefrom'], '<a><font><img><span><strong><b>'));
