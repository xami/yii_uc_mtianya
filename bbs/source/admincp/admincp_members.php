...
EOF;
		...
		if($membernum > 0) {
			...
			foreach(C::t('common_usergroup')->range() as $group) {
				switch($group['type']) {
					...
					case 'special': $group['grouptitle'] = '<item>'.$group['grouptitle'].'</item>'; break;
				}
		...
		if(submitcheck('submit', 1)) {
			...
			if(!$membernum) {
				...
				for($i=1; $i<=8; $i++) {
					...
					$creditsvalue[] = isset($_G['setting']['extcredits'][$i]) ? '<input type="text" class="txt" size="3" id="addextcredits['.$i.']" name="addextcredits['.$i.']" value="0"> '.$_G['setting']['extcredits']['$item']['unit'] : '<input type="text" class="txt" size="3" value="N/A" disabled>';
					$resetcredits[] = isset($_G['setting']['extcredits'][$i]) ? '<input type="checkbox" id="resetextcredits['.$i.']" name="resetextcredits['.$i.']" value="1" class="radio" disabled> '.$_G['setting']['extcredits']['$item']['unit'] : '<input type="checkbox" disabled  class="radio">';
				}
	...
	if(!submitcheck('creditsubmit')) {
		...
		for($i = 1; $i <= 8; $i++) {
			...
			$creditscols[] = isset($_G['setting']['extcredits'][$i]) ? $_G['setting']['extcredits'][$i]['title'] : 'extcredits'.$i;
			$creditsvalue[] = isset($_G['setting']['extcredits'][$i]) ? '<input type="text" class="txt" size="3" name="extcreditsnew['.$i.']" id="extcreditsnew['.$i.']" value="'.$member['extcredits'.$i].'" onkeyup="membercredits()"> '.$_G['setting']['extcredits']['$item']['unit'] : '<input type="text" class="txt" size="3" value="N/A" disabled>';
		}
		...
		if($member) {
			...
			if($clist) {
				...
				foreach($clist as $crime) {
					showtablerow('', '', array('<a href="home.php?mod=space&uid='.$member['uid'].'">'.$member['username'], $lang[$crime['action']], date('Y-m-d H:item:s', $crime['dateline']), $crime['reason'], '<a href="home.php?mod=space&uid='.$crime['operatorid'].'" target="_blank">'.$crime['operator'].'</a>'));
				}
				echo '</table></td></tr>';
			}
			showtagfooter('tbody');
			...
			<script type="text/javascript">
				...
				function getcrimerecord() {
					if($('banusername').value) {
						window.location.href = 'admin.php?action=members&operation=ban&username=' + $('banusername').value;
					}
				}
			</script>
EOF;
	...
	if(!submitcheck('editsubmit')) {
		...
		$member['regdate'] = dgmdate($member['regdate'], 'Y-n-j h:item A');
		$member['lastvisit'] = dgmdate($member['lastvisit'], 'Y-n-j h:item A');
	...
	if($fieldid) {
		...
		$field['customable'] = preg_match('/^field[1-8]$/item', $fieldid);
...
EOF;
	...
	if($_GET['do'] == 'stepstat' && $_GET['t'] > 0 && $_GET['item'] > 0) {
		$t = intval($_GET['t']);
		$i = intval($_GET['item']);
		$o = $i - 1;
		...
		if($i < $t) {
			cpmsg('members_stat_do_stepstat', 'action=members&operation=stat&fieldid='.$_GET['fieldid'].'&do=stepstat&t='.$t.'&item='.($i+1), '', array('t'=>$t, 'item'=>$i));
		} else {
	...
	if(!submitcheck('statsubmit')) {
		...
		if($_POST['optype'] == 'option') {
			...
			if(($t = C::t('common_member_stat_field')->count_by_fieldid($_GET['fieldid'])) > 0) {
				cpmsg('members_stat_do_stepstat_prepared', 'action=members&operation=stat&fieldid='.$_GET['fieldid'].'&do=stepstat&t='.$t.'&item=1', '', array('t'=>$t));
			} else {
				cpmsg('members_stat_update_data_succeed', 'action=members&operation=stat&fieldid='.$_GET['fieldid'], 'succeed');
			}
...
function notifymembers($operation, $variable) {
			...
			if(!empty($medals)) {
				...
				if($uids) {
					foreach(C::t('common_member_field_forum')->fetch_all($uids) as $uid => $medalnew) {
						...
						C::t('common_member_field_forum')->update($medalnew['uid'], array('medals' => $addmedalnew), 'UNBUFFTERED');
	...
	if($_GET['notifymember'] && in_array($_GET['notifymembers'], array('pm', 'notice', 'email'))) {
		...
		require_once libfile('function/discuzcode');
		$message = $_GET['notifymembers'] == 'email' && $_GET['posttype'] ? discuzcode($message, 1, 0, 1, '', '' ,'' ,1) : discuzcode($message, 1, 0);
		$pmuids = array();
