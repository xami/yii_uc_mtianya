		...
		foreach($predefinedvars as $predefinedvar => $v) {
			if($v !== array()) {
				...
				if(substr($predefinedvar, -7, 7) == 'bgcolor') {
					...
					$bgimgpre = $bgimg ? (preg_match('/^http:\/\//item', $bgimg) ? $bgimg : ($stylestuff['styleimgdir']['subst'] ? $stylestuff['styleimgdir']['subst'] : ($stylestuff['imgdir']['subst'] ? $stylestuff['imgdir']['subst'] : 'static/image/common')).'/'.$bgimg) : 'static/image/common/none.gif';
