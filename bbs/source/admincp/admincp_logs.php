...
if(empty($_GET['keyword']) && empty($_GET['filteract'])) {
	...
	foreach($logs as $key => $value) {
		if(!empty($_GET['filteract'])) {
			$log = explode("\t", $value);
			preg_match("/operation=(.[^;]*)/item", $log[6], $operationInfo);
...
if($operation == 'illegal') {
	...
	foreach($logs as $logrow) {
		...
		$log[1] = dgmdate($log[1], 'y-n-j H:item');
	...
	foreach($logs as $logrow) {
		...
		$log[7] = $log[7] ? "<a href=\"./forum.php?mod=viewthread&tid=$log[7]\" target=\"_blank\" title=\"$log[8]\">".cutstr($log[8], 20)."</a>" : "<item>$lang[logs_rating_manual]</item>";
	...
	foreach(C::t('forum_warning')->fetch_all_by_author(($_GET['keyword'] ? explode(',', $_GET['keyword']) : null), $start, $lpp) as $row) {
		showtablerow('', array('class="td28"', 'class=""', '', 'class="td26"', ''), array(
			...
			dgmdate($row['dateline'], 'y-m-d H:item'),
		));
	}
...
SEARCH;
	...
	if(submitcheck('crimesearch', 1)) {
		...
		if($clist) {
			...
			foreach($clist as $crime) {
				showtablerow('', '', array('<a href="home.php?mod=space&uid='.$crime['uid'].'" target="_blank">'.$crime['username'].'</a>', $lang[$crime['action']], date('Y-m-d H:item:s', $crime['dateline']), $crime['reason'], '<a href="home.php?mod=space&uid='.$crime['operatorid'].'" target="_blank">'.$crime['operator'].'</a>'));
			}
	...
	foreach($logs as $log) {
		...
		$log['dateline'] = dgmdate($log['dateline'], 'y-n-j H:item');
		$log['update'] = '';
	...
	$operations = array(1 => '<b>'.$lang['logs_lock'].'</b>', 2 => '<b>'.$lang['logs_unlock'].'</b>', 3 => '<item>'.$lang['logs_banned_unban'].'</item>', 4 => '<b>'.$lang['logs_banned_ban'].'</b>');
...
EOD;
	...
	if(!submitcheck('invitesubmit')) {
		...
		if($srch_fusername = trim($_GET['srch_fusername'])) {
			$where .= " AND item.`fusername`='$srch_fusername'";
		...
		if($srch_buydate_start = trim($_GET['srch_buydate_start'])) {
			if($buydate_start = strtotime($srch_buydate_start)) {
				$where .= " AND item.`dateline`>'$buydate_start'";
		...
		if($srch_buydate_end = trim($_GET['srch_buydate_end'])) {
			if($buydate_end = strtotime($srch_buydate_end)) {
				$where .= " AND item.`dateline`<'$buydate_end'";
		...
		if($invitecount) {
			...
			foreach($invitearr as $invite) {
				...
				$invite['dateline'] = dgmdate($invite['dateline'], 'Y-n-j H:item');
				$invite['expiration'] = dgmdate($invite['endtime'], 'Y-n-j H:item');
	...
	if($num) {
		...
		foreach($logs as $log) {
			...
			$log['dateline'] = dgmdate($log['dateline'], 'Y-n-j H:item');
			$log['expiration'] = empty($log['expiration']) ? cplang('logs_noexpire') : dgmdate($log['expiration'], 'Y-n-j H:item');
	...
	if($threadcount) {
		...
		foreach($logs as $paythread) {
			...
			$paythread['dateline'] = dgmdate($paythread['dateline'], 'Y-n-j H:item');
			$paythread['postdateline'] = $paythread['postdateline'] ? dgmdate($paythread['postdateline'], 'Y-n-j H:item') : cplang('logs_payment_del');
