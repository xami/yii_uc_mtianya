...
<?php
	...
	if(!submitcheck('bbcodessubmit') && !$edit) {
		...
		if(is_array($tagnew)) {
			...
			foreach($tagnew as $id => $val) {
				if(in_array($id, $custom_ids) && !preg_match("/^[0-9a-z]+$/item", $tagnew[$id]) && strlen($tagnew[$id]) < 20) {
		...
		$newtag = $_GET['newtag'];
		if($newtag != '') {
			if(!preg_match("/^[0-9a-z]+$/item", $newtag && strlen($newtag) < 20)) {
				cpmsg('dzcode_edit_tag_invalid', '', 'error');
			}
			$data = array(
				'tag' => $newtag,
		...
		if(!submitcheck('editsubmit')) {
			...
			if(!preg_match("/^[0-9a-z]+$/item", $tagnew)) {
				cpmsg('dzcode_edit_tag_invalid', '', 'error');
	...
	if($do == 'export') {
		...
		dheader('Expires: '.gmdate('D, d M Y H:item:s', TIMESTAMP - 31536000).' GMT');
