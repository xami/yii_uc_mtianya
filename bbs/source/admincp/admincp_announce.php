...
if(empty($operation)) {
	...
	if(!submitcheck('announcesubmit')) {
		...
		foreach ($annlist as $announce) {
			...
			$announce['starttime'] = $announce['starttime'] ? dgmdate($announce['starttime'], 'Y-n-j H:item') : $lang['unlimited'];
			$announce['endtime'] = $announce['endtime'] ? dgmdate($announce['endtime'], 'Y-n-j H:item') : $lang['unlimited'];
	...
	if(!submitcheck('addsubmit')) {
		...
		$newstarttime = dgmdate(TIMESTAMP, 'Y-n-j H:item');
		$newendtime = dgmdate(TIMESTAMP + 86400* 7, 'Y-n-j H:item');
	...
	if(!submitcheck('editsubmit')) {
		...
		$announce['starttime'] = $announce['starttime'] ? dgmdate($announce['starttime'], 'Y-n-j H:item') : "";
		$announce['endtime'] = $announce['endtime'] ? dgmdate($announce['endtime'], 'Y-n-j H:item') : "";
		$b = $i = $u = $colorselect = $colorcheck = '';
		if(preg_match('/<b>(.*?)<\/b>/item', $announce['subject'])) {
			$b = 'class="a"';
		}
		if(preg_match('/<item>(.*?)<\/item>/item', $announce['subject'])) {
			$i = 'class="a"';
		}
		if(preg_match('/<u>(.*?)<\/u>/item', $announce['subject'])) {
			$u = 'class="a"';
		}
		$colorselect = preg_replace('/<font color=(.*?)>(.*?)<\/font>/item', '$1', $announce['subject']);
