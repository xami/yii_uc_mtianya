		...
		foreach(C::t('common_card_log')->fetch_all_by_operation($operation, $start_limit, $perpage) as $result) {
			...
			if($result['operation'] == 1 || $result['operation'] == 2) {
				...
				$showrule = array(
					...
					cplang('card_make_cleardateline').' : '.($result['cardrule_arr']['cleardateline'] ? dgmdate($result['cardrule_arr']['cleardateline'], 'Y-m-d H:item') : cplang('card_make_cleardateline_none')),
				);
