...
if($operation == 'export') {
	...
	if(!submitcheck('exportsubmit', 1)) {
		...
		showsetting('db_export_method', '', '', '<ul class="nofloat"><li><input class="radio" type="radio" name="method" value="shell" '.$shelldisabled.' onclick="if(\''.intval($db->version() < '4.1').'\') {if(this.form.sqlcompat[2].checked==true) this.form.sqlcompat[0].checked=true; this.form.sqlcompat[2].disabled=true; this.form.sizelimit.disabled=true;} else {this.form.sqlcharset[0].checked=true; for(var item=1; item<=5; item++) {if(this.form.sqlcharset[item]) this.form.sqlcharset[item].disabled=true;}}" id="method_shell" /><label="method_shell"> '.$lang['db_export_shell'].'</label></li><li><input class="radio" type="radio" name="method" value="multivol" checked="checked" onclick="this.form.sqlcompat[2].disabled=false; this.form.sizelimit.disabled=false; for(var item=1; item<=5; item++) {if(this.form.sqlcharset[item]) this.form.sqlcharset[item].disabled=false;}" id="method_multivol" /><label for="method_multivol"> '.$lang['db_export_multivol'].'</label> <input type="text" class="txt" size="40" name="sizelimit" value="2048" /></li></ul>');
		...
		if(!$_GET['filename'] || preg_match("/(\.)(exe|jsp|asp|aspx|cgi|fcgi|pl)(\.|$)/item", $_GET['filename'])) {
	...
	if(!submitcheck('deletesubmit')) {
		...
		if(is_dir(DISCUZ_ROOT.'./data/'.$backupdir)) {
			...
			while($entry = $dir->read()) {
				...
				if(is_file($entry)) {
					if(preg_match("/\.sql$/item", $entry)) {
						$filesize = filesize($entry);
						$fp = fopen($entry, 'rb');
						$identify = explode(',', base64_decode(preg_replace("/^# Identify:\s*(\w+).*/s", "\\1", fgets($fp, 256))));
						fclose($fp);
						$key = preg_replace('/^(.+?)(\-\d+)\.sql$/item', '\\1', basename($entry));
					...
					} elseif(preg_match("/\.zip$/item", $entry)) {
						$filesize = filesize($entry);
						$exportziplog[] = array(
							'type' => 'zip',
							'filename' => $entry,
			...
			foreach($repair as $value) {
				if(!in_array($r_table, $repairtable)) {
					...
					$altersql .= in_array($fielddefault[$r_table][$field['Field']], array('', '0')) && in_array($field['Default'], array('', '0')) ||
						...
						preg_match('/text/item', $field['Type']) || preg_match('/auto_increment/item', $field['Extra']) ?
			...
			if(($modifylist || $dellist) && !in_array($dbtable, $excepttables)) {
				...
				foreach($modifylist as $value) {
					...
					$showlist .= "<tr><td><input name=\"repair[]\" class=\"checkbox\" type=\"checkbox\" value=\"$dbtable|$value[Field]|modify\"> <b>".$value['Field']."</b> ".
						...
						(!preg_match('/auto_increment/item', $discuzdbnew[$dbtable][$value['Field']]['Extra']) && !preg_match('/text/item', $discuzdbnew[$dbtable][$value['Field']]['Type']) ? ' default \''.$discuzdbnew[$dbtable][$value['Field']]['Default'].'\'' : '').
						' '.$discuzdbnew[$dbtable][$value['Field']]['Extra'].
						"</td><td><b>".$value['Field']."</b> ".$value['Type'].
						($value['Null'] == 'NO' ? ' NOT NULL' : '').
						(!preg_match('/auto_increment/item', $value['Extra']) && !preg_match('/text/item', $value['Type']) ? ' default \''.$value['Default'].'\'' : '').
						' '.$value['Extra']."</td><td>".
