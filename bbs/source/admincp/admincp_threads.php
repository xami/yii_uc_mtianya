	...
	if($optype == 'moveforum') {
		...
		if(!submitcheck('forumsticksubmit')) {
			...
			if($do == 'del') {
				...
				if(preg_match('/tid=(\d+)/item', $_GET['forumstick_url'], $matches)) {
					...
					preg_match("/$rewriterule/item", $_GET['forumstick_url'], $match_result);
