...
if($_GET['op'] == 'checknewpm') {
	...
	if(!empty($_GET['tradeid'])) {
		...
		if($comment) {
			$comment['comment'] = str_replace(array('[b]', '[/b]', '[/color]'), array(''), preg_replace("/\[color=([#\w]+?)\]/item", '', strip_tags($comment['comment'])));
	...
	dheader('Date: '.gmdate('D, d M Y H:item:s', $val['dateline']).' GMT');
	dheader('Last-Modified: '.gmdate('D, d M Y H:item:s', $val['dateline']).' GMT');
