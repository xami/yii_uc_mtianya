...
if($op == 'block') {
	...
	if(submitcheck('blocksubmit')) {
		...
		$_POST['dateformat'] = in_array($_POST['dateformat'], array('Y-m-d', 'm-d', 'H:item', 'Y-m-d H:item')) ? $_POST['dateformat'] : 'Y-m-d';
...
function block_getdateformats($format='') {
	$formats = array('Y-m-d', 'm-d', 'H:item', 'Y-m-d H:item');
	$return = array();
...
function block_ckeck_summary($summary){
	if($summary) {
		...
		foreach($tags as $tag) {
			preg_match_all('/(<'.$tag.')|(<\/'.$tag.'>)/item', $summary, $all);
