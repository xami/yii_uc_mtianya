...
if($_G['fid'] && $_G['forum']['ismoderator']) {
	...
	if($op == 'editforum') {
		...
		if(!submitcheck('editsubmit')) {
			...
			$forumupdate = true;
			$rulesnew = $alloweditrules ? preg_replace('/on(mousewheel|mouseover|click|load|onload|submit|focus|blur)="[^"]*"/item', '', discuzcode($_GET['rulesnew'], 1, 0, 0, 0, 1, 1, 0, 0, 1)) : $_G['forum']['rules'];
