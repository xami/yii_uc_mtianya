...
(
	...
	'founder_upgrade_backup_file' => '<b>升级将备份原文件，目录为: </b>',
	...
	'home_security_runquery' => '<li>您开启了 <u>运行 SQL 语句</u> 功能。建议您只有在十分必要的时候才开启此它。请修改 config/config_global.php 关闭此功能',
	'home_security_service_info' => '当站点程序出现严重安全漏洞需要立即进行修补的时候，Discuz! 会通过手机短信、QQ、电子邮箱方式及时通知给您。此服务完全免费，且手机仅面向中国移动和中国联通手机用户。<span class="spectitle">隐私保护: Discuz! 郑重承诺，不会将您的手机、QQ、电子邮箱信息以任何方式提供给第三方。</span>',
...
',
	...
	'setting_serveropti_maxspm' => '<span class="spectitle">60 秒最大搜索次数</u></item>',
	...
	'setting_sec_seccode_ttf_comment' => '选择“是”将随机使用 static/image/seccode/font/en/ 目录下的 TTF 字体文件生成验证码文字，选择“否”将随机使用 static/image/seccode/gif/ 目录中的 GIF 图片生成验证码文字。中文图片验证码将随机使用 static/image/seccode/font/channel/ 目录下的 TTF 字体文件，无需进行此设置',
	...
	'setting_datetime_visitbanperiods_comment' => '每天该时间段内用户不能访问站点，请使用 24 小时时段格式，每个时间段一行，如需要也可跨越零点，留空为不限制。例如:<br />每日晚 11:25 到次日早 5:05 可设置为: <item>23:25-5:05</item><br />每日早 9:00 到当日下午 2:30 可设置为: <item>9:00-14:30</item><br />注意: 格式不正确将可能导致意想不到的问题，用户组中如开启“不受时间段限制”的选项，则该组可不被任何时间段设置约束。所有时间段设置均以站点系统默认时区为准，不受用户自定义时区的影响',
	'setting_datetime_postbanperiods' => '禁止发帖时间段',
	'setting_datetime_postbanperiods_comment' => '每天该时间段内用户不能发帖，请使用 24 小时时段格式，每个时间段一行，如需要也可跨越零点，留空为不限制。例如:<br />每日晚 11:25 到次日早 5:05 可设置为: <item>23:25-5:05</item><br />每日早 9:00 到当日下午 2:30 可设置为: <item>9:00-14:30</item><br />注意: 格式不正确将可能导致意想不到的问题，用户组中如开启“不受时间段限制”的选项，则该组可不被任何时间段设置约束。所有时间段设置均以站点系统默认时区为准，不受用户自定义时区的影响',
	...
	'setting_imgwater_image_watermarktext_fontpath_comment' => '填写存放在 static/image/seccode/font/channel/ 或 static/image/seccode/font/en/ 目录下的 TTF 字体文件，支持中文字体。如使用中文 TTF 字体请使用包含完整中文汉字的字体文件',
...
<br />(3)SPH_RANK_NONE，禁用评分的模式，这是最快的模式。实际上这种模式与布尔搜索相同。所有的匹配项都被赋予权重1',
	...
	'threadtype_protect_mode_comment' => '选择图片方式：需要空间GD库支持，内容里有中文需要将中文 TTF 字体文件放到 static/image/seccode/font/channel/ 目录下',
	...
	'threadtype_edit_select_choices_comment' => '只在项目为可选时有效，每行一个字段，等号前面为字段索引(建议用数字)，后面为内容，例如: <br /><item>1 = 光电鼠标<br />2 = 机械鼠标<br />3 = 没有鼠标</item><br /><item>1.1 = 黑色光电鼠标</item><br /><item>1.2 = 红色光电鼠标</item><br /><item>1.2.1 = 蓝牙红色光电鼠标</item><br />注意: <br />1、 "1.2.1 = 蓝牙红色光电鼠标"必须有"1.2 = 红色光电鼠标"和"1 = 光电鼠标"这两项<br />2、 "1.2.1"之间不能有空格<br />3、 字段确定后请勿修改索引和内容的对应关系，但仍可以新增字段。如需调换显示顺序，可以通过移动整行的上下位置来实现',
	'threadtype_edit_choices' => '字段内容',
	'threadtype_edit_choices_comment' => '只在项目为可选时有效，每行一个字段，等号前面为字段索引(建议用数字)，后面为内容，例如: <br /><item>1 = 光电鼠标<br />2 = 机械鼠标<br />3 = 没有鼠标</item><br />注意: 字段确定后请勿修改索引和内容的对应关系，但仍可以新增字段。如需调换显示顺序，可以通过移动整行的上下位置来实现',
	...
	'usergroups_specified_members_leaved_out' => ' ... <item>(自动省略)</item>',
	...
	'feed_global_dateline_comment' => '(格式：yyyy-mm-dd H:item)您可以填写一个将来的日期和时间，那么这条动态会在这个将来的日期到来之前，一直显示在第一位',
	...
	'plugins_edit_vars_extra_comment' => '只在配置类型为“选择(select)”时有效，用于设定选项值。等号前面为选项索引(建议用数字)，后面为内容，例如: <br /><item>1 = 光电鼠标<br />2 = 机械鼠标<br />3 = 没有鼠标</item><br />注意: 选项确定后请勿修改索引和内容的对应关系，但仍可以新增选项。如需调换显示顺序，可以通过移动整行的上下位置来实现',
	...
	'ec_alipay_partner_comment' => '支付宝签约用户请在此处填写支付宝分配给您的合作者身份，签约用户的手续费按照您与支付宝官方的签约协议为准。如果您还未签约，<a href="http://act.life.alipay.com/systembiz/discuz/" target="_blank">请点击这里签约</a>；如果在签约时出现合同模板冲突，请咨询0571-88158090',
