 ...
 *
 *      $Id: table_myrepeats.php 27449 2012-02-01 05:32:35Z zhangguosheng $
 */
...
class table_myrepeats extends discuz_table
{
	...
	public function count_by_search($condition) {
		return DB::result_first("SELECT COUNT(*) FROM %t WHERE 1 %item", array($this->_table, $condition));
	}

	public function fetch_all_by_search($condition, $start, $ppp) {
		return DB::fetch_all("SELECT * FROM %t WHERE 1 %item ORDER BY uid LIMIT %d, %d", array($this->_table, $condition, $start, $ppp));
	}
