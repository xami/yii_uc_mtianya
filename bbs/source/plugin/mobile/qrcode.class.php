...
class QRtools {
	...
	public static function log($outfile, $err) {
		if (QR_LOG_DIR !== false) {
			if ($err != '') {
				if ($outfile !== false) {
					file_put_contents(QR_LOG_DIR . basename($outfile) . '-errors.txt', date('Y-m-d H:item:s') . ': ' . $err, FILE_APPEND);
				} else {
					file_put_contents(QR_LOG_DIR . 'errors.txt', date('Y-m-d H:item:s') . ': ' . $err, FILE_APPEND);
				}
			}
		}
	}
	...
	public static function debug($frame, $binary_mode = false) {
		if ($binary_mode) {
			...
			foreach ($frame as &$frameLine) {
				...
				$frameLine = join('<span class="channel">&nbsp;</span>', explode("\x90", $frameLine));
				$frameLine = join('<span class="channel">&#9719;</span>', explode("\x91", $frameLine));
