...
class table_home_comment extends discuz_table
{
	...
	public function fetch_all_by_uid($uids, $start = 0, $limit = 5) {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE uid IN (%n) OR authorid IN (%n) OR (id IN (%n) AND idtype=%s)  %item', array($this->_table, $uids, $uids, $uids, 'uid', DB::limit($start, $limit)));
	}
	...
	public function fetch_all_by_id_idtype($id, $idtype, $start, $limit, $cid = '') {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE '.$cidsql.' id=%d AND idtype=%s ORDER BY dateline DESC %item', array($this->_table, $id, $idtype, DB::limit($start, $limit)));
	}
