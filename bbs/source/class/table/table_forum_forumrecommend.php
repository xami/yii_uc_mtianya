...
class table_forum_forumrecommend extends discuz_table
{
	...
	public function delete_by_fid($fids, $moderatorid = false) {
		...
		DB::query("DELETE FROM %t WHERE %item %item", array($this->_table, DB::field('fid', $fids), $moderatorid));
	}
	...
	public function fetch_all_by_fid($fid, $position = false, $moderatorid = false, $start = 0, $limit = 0) {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE fid=%d %item %item ORDER BY displayorder %item', array($this->_table, $fid, $position, $moderatorid, $limit));
	}
