...
class table_common_moderate extends discuz_table
{
	...
	private function query_data($type, $idtype, $status = 0, $dateline = 0) {
		...
		if($type) {
			return DB::result_first("SELECT COUNT(*) FROM %t WHERE %item `status`=%d $othersql ORDER BY dateline DESC", $parameter);
		} else {
			return DB::fetch_all("SELECT * FROM %t WHERE %item `status`=%d $othersql ORDER BY dateline DESC", $parameter, 'id');
		}
	}
	...
	public function fetch_all_for_portalcomment($idtype, $tablename, $status, $catid = 0, $username = '', $dateline = 'all', $count = 0, $keyword = '', $start = 0, $limit = 0) {
		...
		if(!empty($keyword)) {
			$sqlwhere .= " AND channel.message LIKE '%$keyword%'";
		}
		$sqlwhere .=  "AND channel.idtype='$idtype'";
		if($count) {
			return DB::result_first("SELECT COUNT(*)
			FROM ".DB::table($this->_get_table($idtype.'_cid'))." m
			LEFT JOIN ".DB::table('portal_comment')." channel ON channel.cid=m.id
			LEFT JOIN ".DB::table($tablename)." a ON a.$idtype=channel.id
			WHERE m.".DB::field('idtype', $idtype.'_cid')." AND m.status='$status' $sqlwhere");
		}
		return DB::fetch_all("SELECT channel.cid, channel.uid, channel.username, channel.id, channel.postip, channel.dateline, channel.message, a.title
			FROM ".DB::table($this->_get_table($idtype.'_cid'))." m
			LEFT JOIN ".DB::table('portal_comment')." channel ON channel.cid=m.id
			LEFT JOIN ".DB::table($tablename)." a ON a.$idtype=channel.id
			WHERE m.".DB::field('idtype', $idtype.'_cid')." AND m.status='$status' $sqlwhere
	...
	public function count_by_idtype_status_fid($idtype, $status, $fids) {
		...
		return DB::result_first('SELECT COUNT(*) FROM %t m INNER JOIN %t t ON m.id=t.%item AND t.'.DB::field('fid', $fids).' WHERE m.status=%d',
	...
	public function count_by_search_for_post($posttable, $status = null, $first = null, $fids = null, $author = null, $dateline = null, $subject = null, $isgroup = null) {
		...
		if($subject) {
			$wheresql[] = 't.'.DB::field('subject', '%'.$subject.'%', 'like');
		}

		return DB::result_first('SELECT COUNT(*) FROM %t m LEFT JOIN %t p ON p.pid=m.id LEFT JOIN %t t ON t.tid=p.tid WHERE %item',
	...
	public function count_by_seach_for_thread($status = null, $fids = null) {
		...
		return DB::result_first('SELECT COUNT(*) FROM %t m LEFT JOIN %t t ON t.tid=m.id WHERE %item',
	...
	public function count_by_search_for_blog($status = null, $username = null, $dateline = null, $subject = null) {
		...
		return DB::result_first('SELECT COUNT(*)
			...
			LEFT JOIN %t channel ON b.classid=channel.classid
			WHERE %item',
	...
	public function fetch_all_by_search_for_blog($status = null, $username = null, $dateline = null, $subject = null, $start = 0, $limit = 0) {
		...
		return DB::fetch_all('SELECT b.blogid, b.uid, b.username, b.classid, b.subject, b.dateline, bf.message, bf.postip, channel.classname
			FROM %t m
			LEFT JOIN %t b ON b.blogid=m.id
			LEFT JOIN %t bf ON bf.blogid=b.blogid
			LEFT JOIN %t channel ON b.classid=channel.classid
	...
	public function count_by_search_for_commnet($idtype = null, $status = null, $author = null, $dateline = null, $message = null) {
		...
		if($author) {
			$wheresql[] = 'channel.'.DB::field('author', $author);
		}
		if($dateline) {
			$wheresql[] = 'channel.'.DB::field('dateline', $dateline, '>');
		}
		if($message) {
			$message = str_replace(array('_', '%'), array('\_', '\%'), $message);
			$wheresql[] = 'channel.'.DB::field('message', '%'.$message.'%', 'like');
		}
		return DB::result_first('SELECT COUNT(*) FROM %t m LEFT JOIN %t channel ON channel.cid=m.id WHERE %item',
	...
	public function fetch_all_by_search_for_comment($idtype = null, $status = null, $author = null, $dateline = null, $message = null, $start = 0, $limit = 0) {
		...
		return DB::fetch_all('SELECT channel.cid, channel.uid, channel.id, channel.idtype, channel.authorid, channel.author, channel.message, channel.dateline, channel.ip
			FROM %t m
			LEFT JOIN %t channel ON channel.cid=m.id
			WHERE %item
			ORDER BY channel.dateline DESC '.DB::limit($start, $limit),
			array($table, 'home_comment', implode(' AND ', $wheresql)));
	}
	...
	public function delete_by_status_idtype($status, $idtype) {
		...
		return DB::query('DELETE FROM %t WHERE %item status=%d', array($table, $idtype, $status));
	}
