...
class table_common_plugin extends discuz_table
{
	...
	public function fetch_all_data($available = false) {
		...
		return DB::fetch_all('SELECT * FROM %t %item ORDER BY available DESC, pluginid DESC', array($this->_table, $available));
	}
	...
	public function fetch_all_by_identifier($identifier) {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE %item', array($this->_table, DB::field('identifier', $identifier)));
	}
