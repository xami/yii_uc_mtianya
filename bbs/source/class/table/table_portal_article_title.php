...
class table_portal_article_title extends discuz_table
{
	...
	public function fetch_all_by_sql($where, $order = '', $start = 0, $limit = 0, $count = 0, $alias = '') {
		...
		if($count) {
			return DB::result_first('SELECT count(*) FROM '.DB::table($this->_table).'  %item %item %item '.DB::limit($start, $limit), array($alias, $where, $order));
		}
		return DB::fetch_all('SELECT * FROM '.DB::table($this->_table).' %item %item %item '.DB::limit($start, $limit), array($alias, $where, $order));
	}
