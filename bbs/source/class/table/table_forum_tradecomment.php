...
class table_forum_tradecomment extends discuz_table
{
	...
	function fetch_all_by_rateeid($rateeid, $type, $dateline = 0) {
		...
		return DB::fetch_all("SELECT * FROM %t WHERE rateeid=%d AND type=%d %item", array($this->_table, $rateeid, $type, $dateline));
	}
	...
	function fetch_all_list($from, $uid, $dateline, $score, $start) {
		...
		return DB::fetch_all("SELECT tc.*, tl.subject, tl.price, tl.credit FROM %t tc LEFT JOIN %t tl USING(orderid) WHERE %item %item %item ORDER BY tc.dateline DESC LIMIT %d, 10",
	...
	function count_list($from, $uid, $dateline, $score) {
		...
		return DB::result_first("SELECT COUNT(*) FROM %t tc WHERE %item %item %item", array($this->_table, $sql, $dateline, $score));
	}
