...
class table_home_pic extends discuz_table
{
	...
	public function fetch_all_by_sql($where = '1', $orderby = '', $start = 0, $limit = 0, $count = 0, $joinalbum = 1) {
		...
		if($count) {
			return DB::result_first("SELECT count(*) FROM ".DB::table($this->_table)." p WHERE %item", array($where));
		}
		return DB::fetch_all("SELECT ".($joinalbum ? 'a.*, ' : '')."p.* FROM ".DB::table($this->_table)." p ".($joinalbum ? "LEFT JOIN ".DB::table('home_album')." a USING(albumid)" : '')." WHERE %item ".($orderby ? "ORDER BY $orderby " : '').DB::limit($start, $limit), array($where));
	}
	...
	public function fetch_by_id_idtype($id) {
		...
		return DB::fetch_first('SELECT * FROM %t WHERE %item', array($this->_table, DB::field('picid', $id)));
	}
