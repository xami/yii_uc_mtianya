...
class table_home_notification extends discuz_table
{
	...
	public function count_by_uid($uid, $new, $type = '') {
		...
		return DB::result_first("SELECT COUNT(*) FROM %t WHERE uid=%d %item %item", array($this->_table, $uid, $type, $new));
	}
	...
	public function fetch_all_by_uid($uid, $new, $type, $start, $perpage) {
		...
		$type = $type ? ' AND '.DB::field('type', $type) : '';
		$new = ' AND '.DB::field('new', $new);
		return DB::fetch_all("SELECT * FROM %t WHERE uid=%d %item %item ORDER BY new DESC, dateline DESC %item", array($this->_table, $uid, $type, $new, DB::limit($start, $perpage)));
	}
}
