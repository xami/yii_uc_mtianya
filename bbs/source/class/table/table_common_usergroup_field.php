...
class table_common_usergroup_field extends discuz_table
{
	...
	public function fetch_all_fields($gid, $fields) {
		...
		return DB::fetch_all('SELECT %item FROM %t %item', array($fieldssql, $this->_table, ($gid ? 'WHERE '.DB::field('groupid', $gid) : '')), $this->_pk);
	}
	...
	public function count_by_field($field, $val, $glue = '=') {
		...
		return DB::result_first('SELECT count(*) FROM %t WHERE %item', array($this->_table, DB::field($field, $val, $glue)));
	}
