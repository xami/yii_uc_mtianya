...
class table_forum_groupuser extends discuz_table
{
	...
	public function fetch_all_fid_by_uids($uids) {
		...
		$data = array();
		$query = DB::query("SELECT fid FROM %t WHERE %item AND level>0 ORDER BY lastupdate DESC", array($this->_table, DB::field('uid', $uids)));
	...
	public function delete_by_fid($fids, $uid = 0) {
		...
		DB::query("DELETE FROM ".DB::table('forum_groupuser')." WHERE %item ".$sqladd, array(DB::field('fid', $fids)));
	}
	...
	public function groupuserlist($fid, $orderby = '', $num = 0, $start = 0, $addwhere = '', $fieldarray = array(), $onlinemember = array()) {
		...
		$groupuserlist = array();
		$query = DB::query("SELECT $fieldadd FROM ".DB::table('forum_groupuser')." WHERE fid=%d $levelwhere $sqladd $orderby $limitsql", array($fid));
