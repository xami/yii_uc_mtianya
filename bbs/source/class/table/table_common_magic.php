...
class table_common_magic extends discuz_table
{
	...
	public function fetch_all_data($available = false) {
		...
		return DB::fetch_all('SELECT * FROM %t %item ORDER BY displayorder', array($this->_table, $available));
	}
	...
	public function count_page($operation) {
		...
		return DB::result_first('SELECT COUNT(*) FROM %t WHERE available=1 %item', array($this->_table, $salevolume));
	}
	...
	public function fetch_all_page($operation, $start, $limit) {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE available=1 %item ORDER BY %item LIMIT %d,%d', array($this->_table, $salevolume, $orderby, $start, $limit));
	}
