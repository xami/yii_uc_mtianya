...
class table_forum_announcement extends discuz_table
{
	...
	public function fetch_all_by_time($time, $type, $bannedids, $startrow, $items) {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE starttime <= %d AND (endtime = \'\' || endtime >= %d) %item ORDER BY displayorder DESC LIMIT %d, %d', array($this->_table, $time, $time, $sql, $startrow, $items), $this->_pk);
	}
