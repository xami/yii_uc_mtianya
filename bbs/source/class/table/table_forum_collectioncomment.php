...
class table_forum_collectioncomment extends discuz_table
{
	...
	public function delete_by_uid($uid) {
		...
		return DB::query("DELETE FROM %t WHERE %item", array($this->_table, DB::field('uid', $uid)));
	}
	...
	public function fetch_all_by_uid($uid) {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE %item', array($this->_table, DB::field('uid', $uid)), $this->_pk);
	}
	...
	public function fetch_all_for_search($cid, $ctid, $username, $uid, $useip, $rate, $message, $starttime, $endtime, $start = 0, $limit = 20) {
		...
		if($start == -1) {
			return DB::result_first("SELECT count(*) FROM %t WHERE %item", array($this->_table, $where));
		}
		return DB::fetch_all("SELECT * FROM %t WHERE %item ORDER BY dateline DESC %item", array($this->_table, $where, DB::limit($start, $limit)));
	}
}
