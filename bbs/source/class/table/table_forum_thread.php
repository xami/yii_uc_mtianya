...
class table_forum_thread extends discuz_table
{
	...
	public function fetch_all_by_displayorder($displayorder = 0, $glue = '>=', $start = 0, $limit = 0, $tableid = 0) {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE %item '.DB::limit($start, $limit), array($this->get_table_name($tableid), DB::field('displayorder', $displayorder, $glue)));
	}

	public function fetch_all_by_authorid($authorid, $start = 0, $limit = 0, $tableid = 0) {
		$authorid = dintval($authorid, true);
		return DB::fetch_all("SELECT * FROM %t WHERE %item ORDER BY dateline DESC ".DB::limit($start, $limit), array($this->get_table_name($tableid), DB::field('authorid', $authorid)), $this->_pk);
	}
	...
	public function fetch_all_by_tid_fid($tids, $fids = array(), $isgroup = -1, $auther = '', $subject = '', $start = 0, $limit = 0) {
		$data = array();
		$condition = $this->make_query_condition($tids, $fids, $isgroup, $auther, $subject);
	...
	private function make_query_condition($tids, $fids = array(), $isgroup = -1, $auther = '', $subject = '', $displayorder = null, $dateline = null) {
		$parameter = array($this->get_table_name());
		$wherearr = array();
		if(!empty($tids)) {
			$tids = dintval($tids, true);
	...
	public function count_search($conditions, $tableid = 0, $prefix = false) {
		$prefix = $prefix ? '' : 't';
		return DB::result_first("SELECT COUNT(*) FROM %t $prefix %item", array($this->get_table_name($tableid), $this->search_condition($conditions, $prefix)));
	}
	...
	public function count_by_tid_fid($tids, $fids = array(), $isgroup = -1, $auther = '', $subject = '') {
		$condition = $this->make_query_condition($tids, $fids, $isgroup, $auther, $subject);
