...
class table_forum_forum extends discuz_table
{
	...
	public function fetch_forum_num($type = '', $fup = '') {
		...
		$addwhere = $type == 'group' ? "`type`='sub' AND `status`='3'" : "`status`<>3";
	...
	public function check_forum_exists($fids, $issub = 1) {
		...
		return DB::result_first("SELECT COUNT(*) FROM ".DB::table($this->_table)." WHERE %item".$typesql, array(DB::field('fid', $fids)));
	}
	...
	public function fetch_all_for_search($conditions, $start = 0, $limit = 20) {
		...
		if($start == -1) {
			...
			WHERE status='3' AND type='sub' AND %item", array($conditions));
		}
		return DB::fetch_all("SELECT f.fid, f.fup, f.type, f.name, f.posts, f.threads, ff.membernum, ff.lastupdate, ff.dateline, ff.foundername, ff.founderuid FROM ".DB::table($this->_table)." f LEFT JOIN ".DB::table('forum_forumfield')." ff ON f.fid=ff.fid
			WHERE status='3' AND type='sub' AND %item ".DB::limit($start, $limit), array($conditions));
	}
	...
	public function fetch_all_fid_for_group($start, $limit, $issub = 0, $conditions = '') {
		...
		return DB::fetch_all("SELECT fid FROM ".DB::table($this->_table)." WHERE status='3' AND $typesql %item ".DB::limit($start, $limit), array($conditions));
	}
	...
	public function fetch_all_for_ranklist($status, $type, $orderfield, $start = 0, $limit = 0, $ignorefids = array()) {
		...
		return DB::fetch_all("SELECT f.* $fields FROM %t f $jointable WHERE f.status=%d $typesql $ignoresql ORDER BY %item DESC ".DB::limit($start, $limit), array($this->_table, $status, $orderfield));
	}
	...
	public function delete_by_fid($fids) {
		...
		DB::query("DELETE FROM ".DB::table($this->_table)." WHERE %item", array(DB::field('fid', $fids)));
		DB::query("DELETE FROM ".DB::table('forum_forumfield')." WHERE %item", array(DB::field('fid', $fids)));
	}
	...
	public function validate_level_for_group($fids) {
		...
		DB::query("UPDATE ".DB::table($this->_table)." SET level='0' WHERE %item", array(DB::field('fid', $fids)));
	}
