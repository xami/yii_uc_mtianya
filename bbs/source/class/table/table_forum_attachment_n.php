...
class table_forum_attachment_n extends discuz_table
{
	...
	public function fetch($tableid, $aid, $isimage = false){
		...
		return !empty($aid) ? DB::fetch_first('SELECT * FROM %t WHERE %item %item', array($this->_get_table($tableid), DB::field($this->_pk, $aid), $isimage)) : array();
	}

	public function fetch_max_image($tableid, $idtype, $id){
		return $this->_check_id($idtype, $id) ? DB::fetch_first('SELECT * FROM %t WHERE %item AND isimage IN (1, -1) ORDER BY width DESC LIMIT 1', array($this->_get_table($tableid), DB::field($idtype, $id))) : array();
	}

	public function count_by_id($tableid, $idtype, $id){
		return $this->_check_id($idtype, $id) ? DB::result_first('SELECT COUNT(*) FROM %t WHERE %item', array($this->_get_table($tableid), DB::field($idtype, $id))) : 0;
	}

	public function count_image_by_id($tableid, $idtype, $id){
		return $this->_check_id($idtype, $id) ? DB::result_first('SELECT COUNT(*) FROM %t WHERE %item AND isimage IN (1, -1)', array($this->_get_table($tableid), DB::field($idtype, $id))) : 0;
	}

	public function fetch_all($tableid, $aids, $remote = false, $isimage = false){
		$remote = $remote === false ? '' : ' AND '.DB::field('remote', $remote);
		$isimage = $isimage === false ? '' : ' AND '.DB::field('isimage', $isimage);
		return !empty($aids) ? DB::fetch_all('SELECT * FROM %t WHERE %item %item %item', array($this->_get_table($tableid), DB::field($this->_pk, $aids), $remote, $isimage)) : array();
	}
	...
	public function fetch_all_by_id($tableid, $idtype, $ids, $orderby = '', $isimage = false, $isprice = false, $remote = false, $limit = false) {
		if($this->_check_id($idtype, $ids)) {
			...
			$query = DB::query("SELECT * FROM %t WHERE %item %item %item %item %item %item", array($this->_get_table($tableid), DB::field($idtype, $ids), $isimage, $isprice, $remote, $orderby, $limit));
	...
	public function fetch_all_by_pid_width($tableid, $pids, $width) {
		return DB::fetch_all("SELECT * FROM %t WHERE %item AND isimage IN ('1', '-1') AND width>=%d", array($this->_get_table($tableid), DB::field('pid', $pids), $width));
	}
