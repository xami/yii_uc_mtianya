...
class table_common_member_crime extends discuz_table
{
	...
	public function count_by_where($where) {
		return $where ? DB::result_first('SELECT COUNT(*) FROM %t %item ', array($this->_table, $where)) : $this->count();
	}

	public function fetch_all_by_where($where, $start = 0, $limit = 0) {
		return DB::fetch_all('SELECT * FROM %t %item ORDER BY dateline DESC '.DB::limit($start, $limit), array($this->_table, $where));
	}
}
