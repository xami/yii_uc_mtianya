...
class table_forum_optionvalue extends discuz_table
{
	...
	public function alter($sortid, $sql) {
		...
		$sortid = intval($sortid);
		$this->_table = 'forum_optionvalue'.$sortid;
		DB::query("ALTER TABLE %t %item", array($this->_table, $sql));
	}
	...
	public function fetch_all_tid($sortid, $where) {
		...
		$sortid = intval($sortid);
		$this->_table = 'forum_optionvalue'.$sortid;
		$query = DB::query("SELECT tid FROM %t %item", array($this->_table, $where));
		$return = array();
	...
	public function update($sortid, $tid, $fid, $fields) {
		...
		$sortid = intval($sortid);
		$this->_table = 'forum_optionvalue'.$sortid;
		DB::query("UPDATE %t SET %item WHERE tid=%d AND fid=%d", array($this->_table, $fields, $tid, $fid));
	}
	...
	public function insert($sortid, $fields, $replace = false) {
		...
		$sortid = intval($sortid);
		$this->_table = 'forum_optionvalue'.$sortid;
		DB::query("%item INTO %t %item", array(!$replace ? 'INSERT' : 'REPLACE', $this->_table, $fields));
	}
