...
class table_common_member_magic extends discuz_table
{
	...
	public function fetch_all($uid, $magicid = '', $start = 0, $limit = 0) {
		...
		return DB::fetch_all('SELECT * FROM %t WHERE %item', array($this->_table, $para.$sql));
	}
	...
	public function fetch($uid, $magicid) {
		...
		return DB::fetch_first('SELECT * FROM %t WHERE %item', array($this->_table, $sql));
	}
	...
	public function count($uid, $magicid) {
		...
		$sql = implode(' AND ', $para);
		return (int) DB::result_first('SELECT count(*) FROM %t WHERE %item', array($this->_table, $sql));
	}
	...
	public function increase($uid, $magicid, $setarr, $slient = false, $unbuffered = false) {
		...
		$sql = implode(' AND ', $para);
		return DB::query('UPDATE %t SET %item WHERE %item', array($this->_table, implode(',', $setsql), $sql), $slient, $unbuffered);
	}
