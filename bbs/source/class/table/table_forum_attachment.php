...
class table_forum_attachment extends discuz_table
{
	...
	public function fetch_all_by_id($idtype, $ids, $orderby = '') {
		...
		if(in_array($idtype, array('aid', 'tid', 'pid', 'uid')) && $ids) {
			$query = DB::query("SELECT * FROM %t WHERE %item IN (%n) %item", array($this->_table, $idtype, (array)$ids, $orderby));
	...
	public function delete_by_id($idtype, $ids) {
		if(in_array($idtype, array('aid', 'tid', 'pid', 'uid')) && $ids) {
			DB::query('DELETE FROM %t WHERE %item IN (%n)', array($this->_table, $idtype, (array)$ids), false, true);
		}
	}

	public function update_by_id($idtype, $ids, $newtid) {
		if(in_array($idtype, array('tid', 'pid')) && $ids) {
			DB::query("UPDATE %t SET tid=%d,tableid=%d WHERE %item IN (%n)", array($this->_table, $newtid, getattachtableid($newtid), $idtype, (array)$ids), false, true);
		}
	}
