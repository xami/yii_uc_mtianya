...
class table_common_style extends discuz_table
{
	...
	public function fetch_all_data($withtemplate = false, $available = false) {
		if($withtemplate) {
			...
			return DB::fetch_all('SELECT s.*, t.name AS tplname, t.directory, t.copyright FROM %t s LEFT JOIN %t t ON t.templateid=s.templateid %item ORDER BY s.available DESC, s.styleid ASC', array($this->_table, 'common_template', $available));
		} else {
			$available = $available !== false ? 'WHERE available='.intval($available) : '';
			return DB::fetch_all('SELECT * FROM %t %item', array($this->_table, $available));
		}
	}
