...
class bbcode {
	...
	function bbcode2html($message, $parseurl=0) {
		if(empty($this->search_exp)) {
			...
			$this->search_str = array('[b]', '[/b]','[item]', '[/item]', '[u]', '[/u]');
			$this->replace_str = array('<b>', '</b>', '<item>','</item>', '<u>', '</u>');
		}
	...
	function html2bbcode($message) {
		...
		if(empty($this->html_s_exp)) {
			...
			$this->html_s_str = array('<b>', '</b>', '<item>','</item>', '<u>', '</u>', '&nbsp; &nbsp; &nbsp; &nbsp; ', '&nbsp; &nbsp;', '&nbsp;&nbsp;', '&lt;', '&gt;', '&amp;');
			$this->html_r_str = array('[b]', '[/b]','[item]', '[/item]', '[u]', '[/u]', "\t", '   ', '  ', '<', '>', '&');
		}
