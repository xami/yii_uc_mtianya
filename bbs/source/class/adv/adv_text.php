		...
		return array(
			...
			'create' => '
				...
				if($advcount > 5) {
	        		...
	        		$adcode = \'\';
	        		for($item = 0; $item < $advcols * ceil($advcount / $advcols); $item++) {
	        			$adcode .= (($item + 1) % $advcols == 1 || $advcols == 1 ? \'<tr>\' : \'\').
	        				\'<td width="\'.intval(100 / $advcols).\'%">\'.(isset($codes[$adids[$item]]) ? $codes[$adids[$item]] : \'&nbsp;\').\'</td>\'.
	        				(($item + 1) % $advcols == 0 ? "</tr>\n" : \'\');
	        		}
