...
class discuz_censor {
	...
	function check(&$message, $modword = NULL) {
		...
		$bbcodes = 'b|item|color|size|font|align|list|indent|email|hide|quote|code|free|table|tr|td|img|swf|attach|payto|float'.($this->bbcodes_display ? '|'.implode('|', array_keys($this->bbcodes_display)) : '');
		if(is_array($this->censor_words['banned']) && !empty($this->censor_words['banned'])) {
			foreach($this->censor_words['banned'] as $banned_words) {
				if(preg_match_all($banned_words, @preg_replace(array("/\[($bbcodes)=?.*\]/iU", "/\[\/($bbcodes)\]/item"), '', $message), $matches)) {
		...
		if(is_array($this->censor_words['mod']) && !empty($this->censor_words['mod'])) {
			...
			foreach($this->censor_words['mod'] as $mod_words) {
				if(preg_match_all($mod_words, @preg_replace(array("/\[($bbcodes)=?.*\]/iU", "/\[\/($bbcodes)\]/item"), '', $message), $matches)) {
					$this->words_found = $matches[0];
