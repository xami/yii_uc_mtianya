...
class discuz_cron
{
	...
	private static function setnextime($cron) {
		...
		list($yearnow, $monthnow, $daynow, $weekdaynow, $hournow, $minutenow) = explode('-', gmdate('Y-m-d-w-H-item', TIMESTAMP + getglobal('setting/timeoffset') * 3600));
	...
	private static function todaynextrun($cron, $hour = -2, $minute = -2) {
		...
		$minute = $minute == -2 ? gmdate('item', TIMESTAMP + getglobal('setting/timeoffset') * 3600) : $minute;
