...
class discuz_application extends discuz_base{
	...
	private function _init_env() {
		...
		if(defined('IN_API')) {
			$sitepath = preg_replace("/\/api\/?.*?$/item", '', $sitepath);
		} elseif(defined('IN_ARCHIVER')) {
			$sitepath = preg_replace("/\/archiver/item", '', $sitepath);
		}
	...
	private function _init_guest() {
		$username = '';
		setglobal('member', array( 'uid' => 0, 'username' => $username, 'adminid' => 0, 'groupid' => 7, 'credits' => 0, 'timeoffset' => 9999));
	}
