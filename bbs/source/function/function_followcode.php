...
function followcode($message, $tid = 0, $pid = 0, $length = 0, $allowimg = true) {
	...
	if(strpos($msglower, '[/item]') !== FALSE) {
		$message = preg_replace("/\s*\[item=s\][\n\r]*(.+?)[\n\r]*\[\/item\]\s*/is", '', $message);
	}
	...
	if(strpos($msglower, '[/p]') !== FALSE) {
		$message = preg_replace("/\[p=(\d{1,2}|null), (\d{1,2}|null), (left|center|right)\]/item", "<p style=\"line-height:\\1px;text-indent:\\2em;text-align:left;\">", $message);
	...
	if($tid && $pid) {
		...
		foreach($_G['post_attach'] as $aid => $attach) {
			...
			$message = preg_replace("/\[attach\]$attach[aid]\[\/attach\]/item", fparseattach($attach['aid'], $length, $extra), $message, 1);
		}
	}
