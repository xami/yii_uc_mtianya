 ...
 *
 *      $Id: function_discuzcode.php 27534 2012-02-03 06:18:27Z monkey $
 */
...
function discuzcode($message, $smileyoff, $bbcodeoff, $htmlon = 0, $allowsmilies = 1, $allowbbcode = 1, $allowimgcode = 1, $allowhtml = 0, $jammer = 0, $parsetype = '0', $authorid = '0', $allowmediacode = '0', $pid = 0, $lazyload = 0, $pdateline = 0) {
	...
	if(!$htmlon) {
		...
		$message = preg_replace("/<script[^\>]*?>(.*?)<\/script>/item", '', $message);
	}
	...
	if(!$bbcodeoff && $allowbbcode) {
		...
		$message = str_replace(array(
			...
			'[item=s]', '[item]', '[/item]', '[u]', '[/u]', '[list]', '[list=1]', '[list=a]',
			'[list=A]', "\r\n[*]", '[*]', '[/list]', '[indent]', '[/indent]', '[/float]'
			), array(
			'</font>', '</font>', '</font>', '</font>', '</div>', '<strong>', '</strong>', '<strike>', '</strike>', '<hr class="l" />', '</p>', '<item class="pstatus">', '<item>',
			'</item>', '<u>', '</u>', '<ul>', '<ul type="1" class="litype_1">', '<ul type="a" class="litype_2">',
			'<ul type="A" class="litype_3">', '<li>', '<li>', '</ul>', '<blockquote>', '</blockquote>', '</span>'
			), preg_replace(array(
			"/\[color=([#\w]+?)\]/item",
			"/\[color=(rgb\([\d\s,]+?\))\]/item",
			"/\[backcolor=([#\w]+?)\]/item",
			"/\[backcolor=(rgb\([\d\s,]+?\))\]/item",
			"/\[size=(\d{1,2}?)\]/item",
			"/\[size=(\d{1,2}(\.\d{1,2}+)?(px|pt)+?)\]/item",
			"/\[font=([^\[\<]+?)\]/item",
			"/\[align=(left|center|right)\]/item",
			"/\[p=(\d{1,2}|null), (\d{1,2}|null), (left|center|right)\]/item",
			"/\[float=left\]/item",
			"/\[float=right\]/item"
				...
				if($authorreplyexist === null) {
					if(!$_G['forum']['ismoderator']) {
						if($_G['uid']) {
							...
							$authorreplyexist = C::t('forum_post')->fetch_pid_by_tid_clientip($_G['tid'], $_G['clientip']);
						}
					} else {
...
function parseurl($url, $text, $scheme) {
	global $_G;
	if(!$url && preg_match("/((https?|ftp|gopher|news|telnet|rtsp|mms|callto|bctp|thunder|qqdl|synacast){1}:\/\/|www\.)[^\[\"']+/item", trim($text), $matches)) {
		$url = $matches[0];
		$length = 65;
...
function parseflash($w, $h, $url) {
	$w = !$w ? 550 : $w;
	$h = !$h ? 400 : $h;
	preg_match("/((https?){1}:\/\/|www\.)[^\[\"']+/item", $url, $matches);
	$url = $matches[0];
...
function parseemail($email, $text) {
	...
	if(!$email && preg_match("/\s*([a-z0-9\-_.+]+)@([a-z0-9\-_]+[.][a-z0-9\-_.]+)\s*/item", $text, $matches)) {
...
function parseflv($url, $width = 0, $height = 0) {
	...
	if($lowerurl != str_replace(array('player.youku.com/player.php/sid/','tudou.com/v/','player.ku6.com/refer/'), '', $lowerurl)) {
		...
		if(preg_match("/http:\/\/v.youku.com\/v_show\/id_([^\/]+)(.html|)/item", $url, $matches)) {
			...
			if(!$width && !$height) {
				...
				if(!empty($str) && preg_match("/\"logo\":\"(.+?)\"/item", $str, $image)) {
		...
		if(preg_match("/http:\/\/(www.)?tudou.com\/programs\/view\/([^\/]+)/item", $url, $matches)) {
			$flv = 'http://www.tudou.com/v/'.$matches[2];
			if(!$width && !$height) {
				$str = file_get_contents($url);
				if(!empty($str) && preg_match("/<span class=\"s_pic\">(.+?)<\/span>/item", $str, $image)) {
					$imgurl = trim($image[1]);
				}
			}
		}
	} elseif(strpos($lowerurl, 'v.ku6.com/show/') !== FALSE) {
		if(preg_match("/http:\/\/v.ku6.com\/show\/([^\/]+).html/item", $url, $matches)) {
			...
			if(!$width && !$height) {
				...
				if(!empty($str) && preg_match("/\"picpath\":\"(.+?)\"/item", $str, $image)) {
					$imgurl = str_replace(array('\u003a', '\u002e'), array(':', '.'), $image[1]);
				}
			}
		}
	} elseif(strpos($lowerurl, 'v.ku6.com/special/show_') !== FALSE) {
		if(preg_match("/http:\/\/v.ku6.com\/special\/show_\d+\/([^\/]+).html/item", $url, $matches)) {
			$flv = 'http://player.ku6.com/refer/'.$matches[1].'/v.swf';
			if(!$width && !$height) {
				$api = 'http://vo.ku6.com/fetchVideo4Player/1/'.$matches[1].'.html';
				$str = file_get_contents($api);
		...
		if(preg_match("/http:\/\/www.youtube.com\/watch\?v=([^\/&]+)&?/item", $url, $matches)) {
			$flv = 'http://www.youtube.com/v/'.$matches[1].'&hl=zh_CN&fs=1';
			if(!$width && !$height) {
				$str = file_get_contents($url);
				if(!empty($str) && preg_match("/'VIDEO_HQ_THUMB':\s'(.+?)'/item", $str, $image)) {
					$url = substr($image[1], 0, strrpos($image[1], '/')+1);
		...
		if(preg_match("/http:\/\/tv.mofile.com\/([^\/]+)/item", $url, $matches)) {
			$flv = 'http://tv.mofile.com/cn/xplayer.swf?v='.$matches[1];
			if(!$width && !$height) {
				$str = file_get_contents($url);
				if(!empty($str) && preg_match("/thumbpath=\"(.+?)\";/item", $str, $image)) {
					$imgurl = trim($image[1]);
				}
			}
		}
	} elseif(strpos($lowerurl, 'v.mofile.com/show/') !== FALSE) {
		if(preg_match("/http:\/\/v.mofile.com\/show\/([^\/]+).shtml/item", $url, $matches)) {
			$flv = 'http://tv.mofile.com/cn/xplayer.swf?v='.$matches[1];
			if(!$width && !$height) {
				$str = file_get_contents($url);
				if(!empty($str) && preg_match("/thumbpath=\"(.+?)\";/item", $str, $image)) {
		...
		if(preg_match("/http:\/\/video.sina.com.cn\/v\/b\/(\d+)-(\d+).html/item", $url, $matches)) {
		...
		if(preg_match("/http:\/\/you.video.sina.com.cn\/b\/(\d+)-(\d+).html/item", $url, $matches)) {
			$flv = 'http://vhead.blog.sina.com.cn/player/outer_player.swf?vid='.$matches[1];
			if(!$width && !$height) {
				$api = 'http://interface.video.sina.com.cn/interface/common/getVideoImage.php?vid='.$matches[1];
				$str = file_get_contents($api);
		...
		$str = file_get_contents($url);
		if(!empty($str) && preg_match("/var\sflv\s=\s'(.+?)';/item", $str, $matches)) {
			$flv = $_G['style']['imgdir'].'/flvplayer.swf?&autostart=true&file='.urlencode($matches[1]);
			if(!$width && !$height && preg_match("/var\simga=\s'(.+?)';/item", $str, $image)) {
				$imgurl = trim($image[1]);
			}
		}
	} elseif(strpos($lowerurl, 'http://www.56.com') !== FALSE) {

		if(preg_match("/http:\/\/www.56.com\/\S+\/play_album-aid-(\d+)_vid-(.+?).html/item", $url, $matches)) {
			$flv = 'http://player.56.com/v_'.$matches[2].'.swf';
			$matches[1] = $matches[2];
		} elseif(preg_match("/http:\/\/www.56.com\/\S+\/([^\/]+).html/item", $url, $matches)) {
		...
		if(!$width && !$height && !empty($matches[1])) {
			...
			$str = file_get_contents($api);
			if(!empty($str) && preg_match("/\"img\":\"(.+?)\"/item", $str, $image)) {
				$imgurl = trim($image[1]);
			}
		}
	}
...
function parseimg($width, $height, $src, $lazyload, $pid, $extra = '') {
	...
	return bbcodeurl($src, '<img id="aimg_'.$rimg_id.'" onclick="zoom(this, this.src, 0, 0, '.($_G['setting']['showexif'] ? 1 : 0).')" class="zoom"'.($width > 0 ? ' width="'.$width.'"' : '').($height > 0 ? ' height="'.$height.'"' : '').' '.$attrsrc.'="'.$src.'" '.$extra.' border="0" alt="" />');
}
