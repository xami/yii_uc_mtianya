...
function feed_mktarget($html) {
	...
	if($html && $_G['setting']['feedtargetblank']) {
		$html = preg_replace("/target\=([\'\"]?)[\w]+([\'\"]?)/item", '', $html);
		$html = preg_replace("/<a(.+?)href=([\'\"]?)([^>\s]+)\\2([^>]*)>/item", '<a href="\\3" target="_blank" \\1 \\4>', $html);
	}
	return $html;
}
			...
			if($value) {
				if($value['friend'] != 3) {
					...
					if($value['friend'] == 4) {
						...
						$value['message'] = preg_replace("/&[a-z]+\;/item", '', $value['message']);
