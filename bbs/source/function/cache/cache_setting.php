...
function build_cache_setting() {
	...
	foreach(C::t('common_setting')->fetch_all_not_key($skipkeys) as $setting) {
		if($setting['skey'] == 'extcredits') {
			...
			$setting['svalue'] = preg_replace("/\.asp|\\0/item", '0', $setting['svalue']);
	...
	foreach($data['watermarktype'] as $k => $v) {
		if($data['watermarktype'][$k] == 'text' && $data['watermarktext']['text'][$k]) {
			...
			} elseif(file_exists('static/image/seccode/font/channel/'.$data['watermarktext']['fontpath'][$k])) {
				$data['watermarktext']['fontpath'][$k] = 'static/image/seccode/font/channel/'.$data['watermarktext']['fontpath'][$k];
			} else {
...
function get_cachedata_mainnav() {
	...
	foreach(C::t('common_nav')->fetch_all_mainnav() as $nav) {
		...
		if($nav['logo']) {
			...
			if(!preg_match("/^".preg_quote(STATICURL, '/')."/item", $navlogo) && !(($valueparse = parse_url($navlogo)) && isset($valueparse['host']))) {
...
function get_cachedata_spacenavs() {
	...
	foreach(C::t('common_nav')->fetch_all_by_navtype(2) as $nav) {
		...
		if($nav['icon']) {
			...
			if(!preg_match("/^".preg_quote(STATICURL, '/')."/item", $navicon) && !(($valueparse = parse_url($navicon)) && isset($valueparse['host']))) {
...
function get_cachedata_mynavs() {
	...
	foreach(C::t('common_nav')->fetch_all_by_navtype(3) as $nav) {
		...
		if($nav['icon']) {
			...
			$navicon = preg_match('/^http:\/\//item', $navicon) ? $navicon : $_G['siteurl'].$navicon;
