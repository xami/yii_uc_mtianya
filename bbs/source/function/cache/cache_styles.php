...
function build_cache_styles() {
	...
	foreach(C::t('common_style')->fetch_all_data(true) as $data) {
		...
		if(strstr($data['boardimg'], ',')) {
			...
			$flash[0] = preg_match('/^http:\/\//item', $flash[0]) ? $flash[0] : $data['styleimgdir'].'/'.$flash[0];
			...
			$data['boardimg'] = preg_match('/^http:\/\//item', $data['boardimg']) ? $data['boardimg'] : $data['styleimgdir'].'/'.$data['boardimg'];
		...
		if($data['extstyle']) {
			...
			foreach($extstyle as $dir) {
				if(file_exists($extstylefile = DISCUZ_ROOT.$data['tpldir'].'/style/'.$dir.'/style.css')) {
					...
					if(preg_match('/\[name\](.+?)\[\/name\]/item', $content, $r1) && preg_match('/\[iconbgcolor](.+?)\[\/iconbgcolor]/item', $content, $r2)) {
	...
	for($i = 0; $i < count($codes); $i++) {
		if($i < 2) {
			if($codes[$i] != '') {
				...
				} elseif(preg_match('/^http:\/\//item', $codes[$i])) {
					$css .= 'url("'.$codes[$i].'") ';
				} else {
...
function writetocsscache($data) {
	...
	while(($entry = readdir($dh)) !== false) {
		if(fileext($entry) == 'css') {
			...
			$cssdata = !preg_match('/^http:\/\//item', $data['styleimgdir']) ? preg_replace("/url\(([\"'])?".preg_quote($data['styleimgdir'], '/')."/item", "url(\\1../../$data[styleimgdir]", $cssdata) : $cssdata;
			$cssdata = !preg_match('/^http:\/\//item', $data['imgdir']) ? preg_replace("/url\(([\"'])?".preg_quote($data['imgdir'], '/')."/item", "url(\\1../../$data[imgdir]", $cssdata) : $cssdata;
			$cssdata = !preg_match('/^http:\/\//item', $data['staticurl']) ? preg_replace("/url\(([\"'])?".preg_quote($data['staticurl'], '/')."/item", "url(\\1../../$data[staticurl]", $cssdata) : $cssdata;
			if($entry == 'module.css') {
