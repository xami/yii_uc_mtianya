...
function getframehtml($data = array()) {
	...
	foreach ((array)$data as $id => $content) {
		...
		if ($flag == 'frame') {
			...
			$html .= '<div id="'.$fattr['name'].'_content" class="tb-channel"></div>';
...
function getcssname($dirname) {
	...
	if($css) {
		preg_match("/\[name\](.+?)\[\/name\]/item", trim($css), $mathes);
...
function checksecurity($str) {
	...
	if(preg_match("/(expression|import|javascript)/item", $str)) {
...
function import_diy($file) {
	...
	$content = preg_replace("/\<\!\-\-\[name\](.+?)\[\/name\]\-\-\>\s+/item", '', $content);
...
function checkprimaltpl($template) {
	...
	if(!$template || preg_match("/(\.)(exe|jsp|asp|aspx|cgi|fcgi|pl)(\.|$)/item", $template)) {
...
function getprimaltplname($filename) {
	...
	if($content) {
		preg_match("/\<\!\-\-\[name\](.+?)\[\/name\]\-\-\>/item", trim($content), $mathes);
		if(!empty($mathes[1])) {
