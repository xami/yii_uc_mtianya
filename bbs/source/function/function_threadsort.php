				...
				if($optionide[$fieldname] == 'range') {
					...
					if(!empty($currentchoices)) {
						foreach($currentchoices as $subkey => $subvalue) {
							if(preg_match('/^'.$value.'\.'.'/item', $subkey) || preg_match('/^'.$value.'$'.'/item', $subkey)) {
								$subvalues[] = $subkey;
							}
						}
					}
		...
		foreach($searchoption as $optionid => $option) {
			...
			if($option['value']) {
				if(in_array($option['type'], array('number', 'radio'))) {
					...
					if(!empty($currentchoices)) {
						foreach($currentchoices as $subkey => $subvalue) {
							if(preg_match('/^'.$option['value'].'/item', $subkey)) {
								$subvalues[] = $subkey;
							}
						}
					}
	...
	if($templatearray && $sortthreadlist) {
		...
		foreach($sortthreadlist as $tid => $option) {
			...
			$stemplate[$sortid][$tid] = preg_replace(
							array("/\{sortname\}/item", "/\{author\}/item", "/\{subject\}/item", "/\[url\](.+?)\[\/url\]/item"),
							array(
...
function showsortmodetemplate($sortid, $fid, $sortoptionarray, $templatearray, $threadlist, $threadids = array()) {
	global $_G;
	...
	foreach($threadlist as $thread) {
		...
		$replaces['{author_verify}'] = $GLOBALS['verify'][$thread['authorid']] ? $GLOBALS['verify'][$thread['authorid']] : '';
...
function threadsortshow($sortid, $tid) {
	...
	if($sortoptionarray) {
		...
		if($templatearray['viewthread']) {
			...
			$typetemplate = preg_replace(array("/\{expiration\}/item"), array($threadexpiration), stripslashes($templatearray['viewthread']));
...
function makevaluepic($value) {
	...
	if(function_exists('imagettftext')) {
		$fontroot = DISCUZ_ROOT.'./static/image/seccode/font/channel/';
		...
		if(!empty($fontname)) {
			$fontfile = DISCUZ_ROOT.'./static/image/seccode/font/channel/'.$fontname;
		}
		if($fontfile) {
