...
function ispluginkey($key) {
	return preg_match("/^[a-z]+[a-z0-9_]*$/item", $key);
}
...
function showtips($tips, $id = 'tips', $display = TRUE, $title = '') {
	$tips = cplang($tips);
	$tips = preg_replace('#</li>\s*<li>#item', '</li><li>', $tips);
	$tmp = explode('</li><li>', substr($tips, 4, -5));
	if(count($tmp) > 4) {
		$tips = '<li>'.$tmp[0].'</li><li>'.$tmp[1].'</li><li id="'.$id.'_more" style="border: none; background: none; margin-bottom: 6px;"><a href="###" onclick="var tiplis = $(\''.$id.'lis\').getElementsByTagName(\'li\');for(var item = 0; item < tiplis.length; item++){tiplis[item].style.display=\'\'}$(\''.$id.'_more\').style.display=\'none\';">'.cplang('tips_all').'...</a></li>';
		foreach($tmp AS $k => $v) {
			if($k > 1) {
...
function showtablerow($trstyle = '', $tdstyle = array(), $tdtext = array(), $return = FALSE) {
	$rowswapclass = '';
	if(!preg_match('/class\s*=\s*[\'"]([^\'"<>]+)[\'"]/item', $trstyle, $matches)) {
		...
		if(is_array($tdtext) && count($tdtext) > 2) {
			...
			$trstyle = preg_replace('/class\s*=\s*[\'"]([^\'"<>]+)[\'"]/item', '', $trstyle);
		}
	}
	...
	if($type == 'radio') {
		...
		for($i = 0; $i <= 1; $i++) {
			if($code[$i] != '') {
				...
				} elseif(preg_match('/^http:\/\//item', $code[$i])) {
					$css .= 'url(\''.$code[$i].'\') ';
				} else {
		...
		$s .= "<input id=\"channel{$colorid}_v\" type=\"text\" class=\"txt\" style=\"float:left; width:210px;\" value=\"$value\" name=\"$varname\" onchange=\"updatecolorpreview('channel{$colorid}')\">\n".
			"<input id=\"channel$colorid\" onclick=\"channel{$colorid}_frame.location='static/image/admincp/getcolor.htm?channel{$colorid}|channel{$colorid}_v';showMenu({'ctrlid':'channel$colorid'})\" type=\"button\" class=\"colorwd\" value=\"\" style=\"background: $background\"><span id=\"channel{$colorid}_menu\" style=\"display: none\"><iframe name=\"channel{$colorid}_frame\" src=\"\" frameborder=\"0\" width=\"210\" height=\"148\" scrolling=\"no\"></iframe></span>\n$extra";
...
function exportdata($name, $filename, $data) {
	...
	$root = array(
		...
		'Time' => dgmdate(TIMESTAMP, 'Y-m-d H:item'),
	...
	dheader('Last-Modified: '.gmdate('D, d M Y H:item:s').' GMT');
