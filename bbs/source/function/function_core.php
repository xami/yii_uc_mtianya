...
function lang($file, $langvar = null, $vars = array(), $default = null) {
	...
	if($path != 'plugin') {
		...
		} elseif(!isset($_G['cache']['pluginlanguage_script'][$file]) && preg_match("/^[a-z]+[a-z0-9_]*$/item", $file)) {
...
function libfile($libname, $folder = '') {
	...
	return preg_match('/^[\w\d\/_]+$/item', $path) ? realpath(DISCUZ_ROOT.$path.'.php') : false;
}
...
function pluginmodule($pluginid, $type) {
	...
	if(empty($identifier) || !preg_match("/^[a-z]+[a-z0-9_]*\/$/item", $directory) || !preg_match("/^[a-z0-9_\-]+$/item", $module)) {
...
function periodscheck($periods, $showmessage = 1) {
	...
	if(!$_G['group']['disableperiodctrl'] && $_G['setting'][$periods]) {
		$now = dgmdate(TIMESTAMP, 'G.item', $_G['setting']['timeoffset']);
	...
	if(!$return) {
		...
		foreach($types as $i => $v) {
			...
			if(strpos($useragent, $v) !== false) {
				preg_match('/'.$v.'(\/|\s)([\d\.]+)/item', $useragent, $matches);
				$ver = $matches[2];
