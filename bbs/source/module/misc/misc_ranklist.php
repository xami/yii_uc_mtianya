...
function getranklist_member_blog($num) {
	...
	$blogs = array();
	$sql = "SELECT m.uid,m.username,m.videophotostatus,m.groupid,channel.blogs FROM ".DB::table('common_member').
			" m LEFT JOIN ".DB::table('common_member_count')." channel ON m.uid=channel.uid WHERE channel.blogs>0 ORDER BY blogs DESC LIMIT 0, $num";
...
function getranklist_member_gender($gender, $num = 20) {
	...
	$num = $num ? $num : 20;
	$users = array();
	$query = DB::query("SELECT channel.uid, channel.views FROM ".DB::table('common_member_count')." channel
			LEFT JOIN ".DB::table('common_member_profile')." p ON channel.uid=p.uid
			WHERE channel.views>0 AND p.gender = '$gender' ORDER BY channel.views DESC LIMIT 0, $num");
