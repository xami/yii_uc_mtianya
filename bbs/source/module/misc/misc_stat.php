...
function getstatvars_team() {
	...
	if(is_array($team)) {
		foreach($team['members'] as $uid => $member) {
			...
			@$team['members'][$uid]['offdays'] = $member['offdays'] > $team['avgoffdays'] ? '<b><item>'.$member['offdays'].'</item></b>' : $member['offdays'];
			@$team['members'][$uid]['thismonthposts'] = $member['thismonthposts'] < $team['avgthismonthposts'] / 2 ? '<b><item>'.$member['thismonthposts'].'</item></b>' : $member['thismonthposts'];
			@$team['members'][$uid]['lastactivity'] = dgmdate($member['lastactivity'] + $timeoffset * 3600, 'd');
			@$team['members'][$uid]['thismonthol'] = $member['thismonthol'] < $team['avgthismonthol'] / 2 ? '<b><item>'.$member['thismonthol'].'</item></b>' : $member['thismonthol'];
			@$team['members'][$uid]['totalol'] = $member['totalol'] < $team['avgtotalol'] / 2 ? '<b><item>'.$member['totalol'].'</item></b>' : $member['totalol'];
			@$team['members'][$uid]['modposts'] = $member['modposts'] < $team['avgmodposts'] / 2 ? '<b><item>'.intval($member['modposts']).'</item></b>' : intval($member['modposts']);
			@$team['members'][$uid]['modactions'] = $member['modactions'] < $team['avgmodactions'] / 2 ? '<b><item>'.intval($member['modactions']).'</item></b>' : intval($member['modactions']);
...
function getstatvars_modworks() {
	...
	if($uid) {
		...
		foreach($members as $id => $member) {
			...
			$members[$id]['username'] = ($members[$id]['total'] < $avgmodactioncount / 2) ? ('<b><item>'.$members[$id]['username'].'</item></b>') : ($members[$id]['username']);
		}
