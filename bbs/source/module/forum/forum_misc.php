...
if($_GET['action'] == 'paysucceed') {
	...
	$totalcomment = preg_replace('/<item>([\.\d]+)<\/item>/e', "'<item class=\"cmstarv\" style=\"background-position:20px -'.(intval(\\1) * 16).'px\">'.sprintf('%1.1f', \\1).'</item>'.(\$cic++ % 2 ? '<br />' : '');", $totalcomment);
...
if($_GET['action'] == 'votepoll' && submitcheck('pollsubmit', 1)) {
	...
	foreach($query as $options) {
		...
		$options['polloption'] = preg_replace("/\[url=(https?){1}:\/\/([^\[\"']+?)\](.+?)\[\/url\]/item",
		...
		if($activity['ufield']) {
			...
			if(!empty($activity['ufield']['userfield'])) {
				...
				foreach($_POST as $key => $value) {
					...
					if($_G['cache']['profilesetting'][$key]['formtype'] == 'file' && !preg_match("/^https?:\/\/(.*)?\.(jpg|png|gif|jpeg|bmp)$/item", $value)) {
