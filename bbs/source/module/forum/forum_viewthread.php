...
if(empty($postlist)) {
	...
	foreach($postlist as $pid => $post) {
		$postlist[$pid]['message'] = preg_replace("/\[attach\]\d+\[\/attach\]/item", '', $postlist[$pid]['message']);
	}
}
...
function viewthread_procpost($post, $lastvisit, $ordertype, $maxposition = 0) {
	...
	$post['dateline'] = dgmdate($post['dateline'], 'u', '9999', getglobal('setting/dateformat').' H:item:s');
	...
	if($post['attachment']) {
		if($_G['group']['allowgetattach'] || $_G['group']['allowgetimage']) {
			...
			$post['attachment'] = 0;
			if(preg_match_all("/\[attach\](\d+)\[\/attach\]/item", $post['message'], $matchaids)) {
				$_G['forum_attachtags'][$post['pid']] = $matchaids[1];
			}
		} else {
			$post['message'] = preg_replace("/\[attach\](\d+)\[\/attach\]/item", '', $post['message']);
		}
	}
...
function viewthread_loadcache() {
	...
	if($threadcachemark < $_G['forum']['threadcaches']) {
		...
		if(TIMESTAMP - $threadcache['filemtime'] > $_G['setting']['cachethreadlife']) {
			...
			$_G['setting']['debug'] ? die('<script type="text/javascript">document.getElementById("debuginfo").innerHTML = " '.($_G['setting']['debug'] ? 'Updated at '.gmdate("H:item:s", $threadcache['filemtime'] + 3600 * 8).', Processed in '.$debuginfo['time'].' second(s), '.$debuginfo['queries'].' Queries'.($_G['gzipcompress'] ? ', Gzip enabled' : '') : '').'";</script>') : die();
		}
	}
}
