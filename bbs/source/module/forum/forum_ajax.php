...
if($_GET['action'] == 'checkusername') {
	...
	$censorexp = '/^('.str_replace(array('\\*', "\r\n", ' '), array('.*', '|', ''), preg_quote(($_G['setting']['censoruser'] = trim($_G['setting']['censoruser'])), '/')).')$/item';
		...
		foreach($temp as $value) {
			...
			if(strlen($imageurl)) {
				...
				if(!isset($existentimg[$hash])) {
					...
					$content = '';
					if(preg_match('/^(http:\/\/|\.)/item', $imageurl)) {
						$content = dfsockopen($imageurl);
					} elseif(preg_match('/^('.preg_quote(getglobal('setting/attachurl'), '/').')/item', $imageurl)) {
