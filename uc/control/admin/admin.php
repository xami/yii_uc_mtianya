			...
			if(UC_FOUNDERPW == md5(md5($oldpw).UC_FOUNDERSALT)) {
				...
				if(!is_writable($configfile)) {
					...
					if($newpw != $newpw2) {
						...
						$config = preg_replace("/define\('UC_FOUNDERSALT',\s*'.*?'\);/item", "define('UC_FOUNDERSALT', '$salt');", $config);
						$config = preg_replace("/define\('UC_FOUNDERPW',\s*'.*?'\);/item", "define('UC_FOUNDERPW', '$md5newpw');", $config);
