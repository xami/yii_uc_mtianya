...
class uccode {
	...
	function complie($message) {
		...
		$message = str_replace(array(
			...
			'[item]', '[/item]', '[u]', '[/u]', '[list]', '[list=1]', '[list=a]',
			'[list=A]', '[*]', '[/list]', '[indent]', '[/indent]', '[/float]'
		), array(
			'</font>', '</font>', '</font>', '</p>', '<strong>', '</strong>', '<item>',
			'</item>', '<u>', '</u>', '<ul>', '<ul type="1">', '<ul type="a">',
			'<ul type="A">', '<li>', '</ul>', '<blockquote>', '</blockquote>', '</span>'
		), preg_replace(array(
			"/\[color=([#\w]+?)\]/item",
			"/\[size=(\d+?)\]/item",
			"/\[size=(\d+(\.\d+)?(px|pt|in|cm|mm|pc|em|ex|%)+?)\]/item",
			"/\[font=([^\[\<]+?)\]/item",
			"/\[align=(left|center|right)\]/item",
			"/\[float=(left|right)\]/item"
		), array(
			"<font color=\"\\1\">",
			"<font size=\"\\1\">",
	...
	function parseurl($url, $text) {
		if(!$url && preg_match("/((https?|ftp|gopher|news|telnet|rtsp|mms|callto|bctp|ed2k|thunder|synacast){1}:\/\/|www\.)[^\[\"']+/item", trim($text), $matches)) {
			$url = $matches[0];
			$length = 65;
